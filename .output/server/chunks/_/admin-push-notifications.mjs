...
function dailySessionAlertThresholds(value) {
  const thresholds = [];
  if (value >= 1e3) thresholds.push(1e3);
...
async function currentCandidates() {
  ...
  return [
    ...accountAlertThresholds(accountCount).map((threshold) => ({
      ...
      type: "learner_accounts",
      threshold,
      observed: accountCount,
      payload: {
        title: "Tatitotu \xB7 Nouveau palier",
        body: `${threshold.toLocaleString("fr-CH")} comptes ont \xE9t\xE9 cr\xE9\xE9s.`,
...
async function sendAlert(alert) {
  ...
  const [vapidRows] = await database.query("SELECT public_key, private_key FROM admin_push_vapid WHERE id=1");
  const vapid = vapidRows[0];
  if (!vapid) throw new Error("Cl\xE9s Web Push indisponibles");
  ...
  await Promise.all(enabledSubscriptions.map(async (subscription) => {
    try {
      ...
      const message = error instanceof Error ? error.message.slice(0, 255) : "\xC9chec de livraison Web Push";
  ...
  if (delivered > 0) {
    ...
    await database.execute(
      "UPDATE admin_push_alerts SET status='skipped', last_error='Tous les abonnements ont expir\xE9' WHERE alert_key=?",
      [alert.alert_key]
    );
  }
}
...
function evaluateAdminPushAlerts(options = {}) {
  if (!options.force && Date.now() - lastSessionEvaluationAt < 3e4) return Promise.resolve();
  if (evaluationPromise) return evaluationPromise;
  lastSessionEvaluationAt = Date.now();
  evaluationPromise = evaluate().catch((error) => console.error("[push] \xC9valuation des alertes impossible.", error)).finally(() => {
...
async function initializeAdminPushBaseline() {
  const [[subscriptions]] = await useDatabase().query(
    "SELECT COUNT(*) AS value FROM admin_push_subscriptions WHERE enabled=1"
  );
  if (!Number(subscriptions == null ? void 0 : subscriptions.value)) await insertCandidates("skipped");
}
async function getAdminPushPublicKey() {
  var _a;
  const [rows] = await useDatabase().query("SELECT public_key, private_key FROM admin_push_vapid WHERE id=1");
