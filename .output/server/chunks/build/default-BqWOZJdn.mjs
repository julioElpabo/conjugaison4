...
const copies = {
  fr: {
    ...
    intro: "Votre avis sur le site compte beaucoup. \xC9crivez-moi si vous avez constat\xE9 une erreur, si quelque chose pourrait \xEAtre am\xE9lior\xE9, ou simplement pour me dire que le site vous pla\xEEt.",
    ...
    success: "Il a bien \xE9t\xE9 envoy\xE9. Je le lirai d\xE8s que possible.",
    error: "Le message n\u2019a pas pu \xEAtre envoy\xE9. Veuillez r\xE9essayer dans quelques instants.",
    rateLimited: "La limite d\u2019envoi est atteinte. Veuillez r\xE9essayer plus tard.",
    shortRateLimited: "La limite de {maximum} messages sur la p\xE9riode courte est atteinte. Vous pourrez r\xE9essayer {when}.",
    dailyRateLimited: "La limite de {maximum} messages sur 24 heures est atteinte. Vous pourrez r\xE9essayer {when}.",
    unavailable: "Le formulaire de contact est temporairement indisponible."
  },
  de: {
    title: "An TATITOTU schreiben",
    intro: "Ihre Meinung zu dieser Website ist mir sehr wichtig. Schreiben Sie mir, wenn Sie einen Fehler festgestellt haben, wenn etwas verbessert werden k\xF6nnte oder einfach, um mir zu sagen, dass Ihnen die Website gef\xE4llt.",
    privacy: "Ihre E-Mail-Adresse wird ausschlie\xDFlich verwendet, um Ihnen zu antworten. Um Missbrauch zu vermeiden, ist die Anzahl der Nachrichten begrenzt.",
    ...
    success: "Sie wurde erfolgreich gesendet. Ich werde sie so bald wie m\xF6glich lesen.",
    error: "Die Nachricht konnte nicht gesendet werden. Bitte versuchen Sie es in K\xFCrze erneut.",
    rateLimited: "Das Sendelimit ist erreicht. Bitte versuchen Sie es sp\xE4ter erneut.",
    shortRateLimited: "Das Limit von {maximum} Nachrichten im kurzen Zeitraum ist erreicht. Sie k\xF6nnen es {when} erneut versuchen.",
    dailyRateLimited: "Das Limit von {maximum} Nachrichten innerhalb von 24 Stunden ist erreicht. Sie k\xF6nnen es {when} erneut versuchen.",
  ...
  it: {
    ...
    privacy: "Il tuo indirizzo e-mail viene utilizzato esclusivamente per risponderti. Per limitare gli abusi, il numero di messaggi \xE8 ristretto.",
    ...
    success: "\xC8 stato inviato correttamente. Lo legger\xF2 appena possibile.",
    error: "Non \xE8 stato possibile inviare il messaggio. Riprova tra poco.",
    rateLimited: "\xC8 stato raggiunto il limite di invio. Riprova pi\xF9 tardi.",
    shortRateLimited: "\xC8 stato raggiunto il limite di {maximum} messaggi nel periodo breve. Potrai riprovare {when}.",
    dailyRateLimited: "\xC8 stato raggiunto il limite di {maximum} messaggi in 24 ore. Potrai riprovare {when}.",
    unavailable: "Il modulo di contatto \xE8 temporaneamente non disponibile."
  },
  es: {
    title: "Escribir a TATITOTU",
    intro: "Tu opini\xF3n sobre el sitio cuenta mucho. Escr\xEDbeme si has detectado un error, si crees que algo podr\xEDa mejorarse o simplemente para decirme que te gusta el sitio.",
    privacy: "Tu direcci\xF3n de correo electr\xF3nico solo se utiliza para responderte. Para limitar los abusos, el n\xFAmero de mensajes est\xE1 restringido.",
    ...
    success: "Se ha enviado correctamente. Lo leer\xE9 lo antes posible.",
    error: "No se ha podido enviar el mensaje. Int\xE9ntalo de nuevo en unos instantes.",
    rateLimited: "Se ha alcanzado el l\xEDmite de env\xEDos. Int\xE9ntalo de nuevo m\xE1s tarde.",
    shortRateLimited: "Se ha alcanzado el l\xEDmite de {maximum} mensajes en el periodo corto. Podr\xE1s volver a intentarlo {when}.",
    dailyRateLimited: "Se ha alcanzado el l\xEDmite de {maximum} mensajes en 24 horas. Podr\xE1s volver a intentarlo {when}.",
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))} data-v-216b55c5><section class="contact-dialog__card" data-v-216b55c5><button class="contact-dialog__close" type="button"${ssrRenderAttr("aria-label", unref(copy).close)}${ssrRenderAttr("title", unref(copy).close)}${ssrIncludeBooleanAttr(unref(submitting)) ? " disabled" : ""} data-v-216b55c5><span aria-hidden="true" data-v-216b55c5>×</span></button>`);
      if (unref(sent)) {
        _push(`<div class="contact-dialog__success" role="status" data-v-216b55c5><span class="contact-dialog__success-icon" aria-hidden="true" data-v-216b55c5>✓</span><h2 id="contact-success-title" data-v-216b55c5>${ssrInterpolate(unref(copy).successTitle)}</h2><p data-v-216b55c5>${ssrInterpolate(unref(copy).success)}</p><button class="contact-dialog__primary" type="button" data-v-216b55c5>${ssrInterpolate(unref(copy).close)}</button></div>`);
      } else {
        _push(`<!--[--><header class="contact-dialog__header" data-v-216b55c5><span class="contact-dialog__eyebrow" data-v-216b55c5>CONTACT</span><h2 id="contact-title" data-v-216b55c5>${ssrInterpolate(unref(copy).title)}</h2><p data-v-216b55c5>${ssrInterpolate(unref(copy).intro)}</p></header><form class="contact-dialog__form" data-v-216b55c5><label data-v-216b55c5><span data-v-216b55c5>${ssrInterpolate(unref(copy).email)}</span><input${ssrRenderAttr("value", unref(email))} type="email" maxlength="254" autocomplete="email"${ssrRenderAttr("placeholder", unref(copy).emailPlaceholder)} required data-v-216b55c5></label><label data-v-216b55c5><span data-v-216b55c5>${ssrInterpolate(unref(copy).subject)}</span><input${ssrRenderAttr("value", unref(subject))} type="text"${ssrRenderAttr("minlength", unref(constraints).subjectMinLength)}${ssrRenderAttr("maxlength", unref(constraints).subjectMaxLength)}${ssrRenderAttr("placeholder", unref(copy).subjectPlaceholder)} required data-v-216b55c5></label><label data-v-216b55c5><span data-v-216b55c5>${ssrInterpolate(unref(copy).message)}</span><textarea${ssrRenderAttr("minlength", unref(constraints).messageMinLength)}${ssrRenderAttr("maxlength", unref(constraints).messageMaxLength)} rows="6"${ssrRenderAttr("placeholder", unref(copy).messagePlaceholder)} required data-v-216b55c5>${ssrInterpolate(unref(message))}</textarea><small data-v-216b55c5>${ssrInterpolate(unref(copy).characters.replace("{count}", String(unref(constraints).messageMinLength)))}</small></label><label class="contact-dialog__honeypot" aria-hidden="true" data-v-216b55c5> Site internet <input${ssrRenderAttr("value", unref(website))} name="website" type="text" tabindex="-1" autocomplete="off" data-v-216b55c5></label>`);
        if (unref(turnstileSiteKey)) {
          _push(`<div class="cf-turnstile"${ssrRenderAttr("data-sitekey", unref(turnstileSiteKey))} data-action="contact" data-theme="auto" data-v-216b55c5></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(errorMessage)) {
          _push(`<p class="contact-dialog__error" role="alert" data-v-216b55c5>${ssrInterpolate(unref(errorMessage))}</p>`);
        } else if (!unref(constraints).enabled) {
          _push(`<p class="contact-dialog__error" role="status" data-v-216b55c5>${ssrInterpolate(unref(copy).unavailable)}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<p class="contact-dialog__privacy" data-v-216b55c5>${ssrInterpolate(unref(copy).privacy)}</p><div class="contact-dialog__actions" data-v-216b55c5><button type="button"${ssrIncludeBooleanAttr(unref(submitting)) ? " disabled" : ""} data-v-216b55c5>${ssrInterpolate(unref(copy).cancel)}</button><button class="contact-dialog__primary" type="submit"${ssrIncludeBooleanAttr(unref(submitting) || !unref(constraints).enabled) ? " disabled" : ""} data-v-216b55c5>${ssrInterpolate(unref(submitting) ? unref(copy).sending : unref(copy).send)}</button></div></form><!--]-->`);
      }
...
const __nuxt_component_1 = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main$1, [["__scopeId", "data-v-216b55c5"]]), { __name: "ContactDialog" });
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</main><footer class="site-footer"><p>${ssrInterpolate(unref(ui)("Un outil gratuit pour travailler la conjugaison française."))}</p><div class="site-footer__links"><button type="button">${ssrInterpolate(unref(ui)("Contact"))}</button>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
...
//# sourceMappingURL=default-BqWOZJdn.mjs.map
