...
<template>
  ...
  >
    ...
    >
      <section v-if="revealedAnswers.length" class="coach-help-revealed-answers" aria-live="polite">
        <strong>{{ ui('Réponse après la première tentative') }}</strong>
        <div>
          <span v-for="answer in revealedAnswers" :key="answer">{{ answer }}</span>
        </div>
      </section>
