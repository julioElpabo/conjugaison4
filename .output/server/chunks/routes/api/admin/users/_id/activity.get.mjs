...
const activity_get = defineEventHandler(async (event) => {
  ...
  const events = [
    ...loginRows.map((row) => ({
      ...
      type: row.eventType === "registration" ? "registration" : row.eventType === "login" ? "login" : "account",
  ...
  ].sort((left, right) => new Date(right.occurredAt).getTime() - new Date(left.occurredAt).getTime()).slice(0, 200);
  return {
    summary: {
      createdAt: account.createdAt,
      lastLoginAt: account.lastLoginAt,
      lastSeenAt: account.lastSeenAt,
      loginCount: Number((summary == null ? void 0 : summary.loginCount) || 0),
      exerciseCount: Number((summary == null ? void 0 : summary.exerciseCount) || 0),
      correctCount: Number((summary == null ? void 0 : summary.correctCount) || 0),
      incorrectCount: Number((summary == null ? void 0 : summary.incorrectCount) || 0)
    },
    events
  };
});
