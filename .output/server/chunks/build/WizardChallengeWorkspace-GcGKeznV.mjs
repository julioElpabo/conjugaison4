...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    function loadClassicExercise() {
      classicExercisePromise ??= import('./ClassicExercise-fm0AXzoV.mjs').then((module) => module.default).catch((error) => {
    ...
    function loadChatExercise() {
      chatExercisePromise ??= import('./ChatExercise-BwJcOO4F.mjs').then((module) => module.default).catch((error) => {
    ...
    watch(isPrintOpen, async (open) => {
      ...
      printPreviewComponent.value = markRaw((await import('./PrintPreview-BUZ1SuUP.mjs')).default);
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "wizard-entry-page" }, _attrs))} data-v-f629f9be><div class="challenge-page wizard-page" data-v-f629f9be>`);
      if (!props.startAtLaunch) {
        _push(`<header class="wizard-hero" data-v-f629f9be>`);
        if (unref(currentStep) === 0 && !unref(falcMode)) {
          _push(`<p class="wizard-hero__brand" data-v-f629f9be>${ssrInterpolate(unref(heroTitle))}</p>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(currentStep) === 0 && !unref(falcMode) && !props.embedded) {
          _push(`<h1 class="wizard-hero__subtitle" data-v-f629f9be>${ssrInterpolate(props.homeHeading || unref(ui)("Exercices de conjugaison française, gratuits et sans publicité"))}</h1>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(currentStep) === 0 && !unref(falcMode) && props.embedded) {
          _push(`<h2 class="wizard-hero__subtitle" data-v-f629f9be>${ssrInterpolate(props.homeHeading || unref(ui)("Exercices de conjugaison française, gratuits et sans publicité"))}</h2>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(currentStep) !== 0 && !unref(falcMode) && !props.embedded) {
          _push(`<h1 class="${ssrRenderClass({ "wizard-hero__preset": unref(isPrefilledChallenge) })}" data-v-f629f9be>${ssrInterpolate(unref(heroTitle))}</h1>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(currentStep) !== 0 && !unref(falcMode) && props.embedded) {
          _push(`<h2 class="${ssrRenderClass({ "wizard-hero__preset": unref(isPrefilledChallenge) })}" data-v-f629f9be>${ssrInterpolate(unref(heroTitle))}</h2>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(currentStep) === 0 && !unref(falcMode) && !unref(guidedTourDisabled)) {
          _push(`<button class="tour-entry-button" type="button" data-v-f629f9be><span aria-hidden="true" data-v-f629f9be>?</span>${ssrInterpolate(unref(tourCopy).discover)}</button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</header>`);
      ...
      _push(`<main class="wizard-shell" data-v-f629f9be>`);
      if (unref(catalogueStatus) === "loading") {
        _push(`<div class="page-state" role="status" data-v-f629f9be><span class="loader" aria-hidden="true" data-v-f629f9be></span> ${ssrInterpolate(unref(ui)("Chargement du catalogue de conjugaison…"))}</div>`);
      } else if (unref(catalogueStatus) === "error") {
        _push(`<div class="page-state page-state--error" role="alert" data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(unref(ui)("Le catalogue n’a pas pu être chargé."))}</strong><span data-v-f629f9be>${ssrInterpolate(unref(catalogueError))}</span><button class="primary-button" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("Réessayer"))}</button></div>`);
      } else {
        _push(`<!--[-->`);
        if (unref(actionError)) {
          _push(`<p class="workspace-message workspace-message--error" role="alert" data-v-f629f9be>${ssrInterpolate(unref(actionError))}</p>`);
        } else if (unref(notice)) {
          _push(`<p class="workspace-message workspace-message--success" aria-live="polite" data-v-f629f9be>${ssrInterpolate(unref(notice))}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<section class="${ssrRenderClass([{ "wizard-panel--autocomplete-open": unref(currentStep) === 1 }, "wizard-panel"])}" tabindex="-1" aria-labelledby="wizard-title" data-v-f629f9be><h2 id="wizard-title" class="sr-only" data-v-f629f9be>${ssrInterpolate(unref(ui)("Composer un défi personnalisé"))}</h2>`);
        if (unref(currentStep) !== 0) {
          _push(`<nav class="${ssrRenderClass([{ "wizard-steps--falc": unref(falcMode) }, "wizard-steps"])}" data-tour="wizard-steps"${ssrRenderAttr("aria-label", unref(ui)("Étapes de création du défi"))} data-v-f629f9be><button data-tour-wizard-step="1" class="${ssrRenderClass([{
            "is-active": unref(currentStep) === 1,
            "is-complete": unref(stepStatus).verbs > 0,
            "tour-secondary-focus": unref(tourSecondaryWizardStep) === 1
          }, "wizard-step-tab wizard-step-tab--verbs"])}" type="button" data-v-f629f9be><span data-v-f629f9be>1</span><span data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(unref(ui)("Verbes"))}</strong>`);
          if (!unref(falcMode)) {
            _push(`<small data-v-f629f9be>${ssrInterpolate(unref(stepStatus).verbs ? unref(ui)(unref(stepStatus).verbs > 1 ? "{count} choisis" : "{count} choisi", { count: unref(stepStatus).verbs }) : unref(ui)("À choisir"))}</small>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</span></button><span class="wizard-steps__line" aria-hidden="true" data-v-f629f9be></span><button data-tour-wizard-step="2" class="${ssrRenderClass([{
            "is-active": unref(currentStep) === 2,
            "is-complete": unref(stepStatus).tenses > 0,
            "tour-secondary-focus": unref(tourSecondaryWizardStep) === 2
          }, "wizard-step-tab wizard-step-tab--tenses"])}" type="button"${ssrIncludeBooleanAttr(unref(stepStatus).verbs === 0) ? " disabled" : ""} data-v-f629f9be><span data-v-f629f9be>2</span><span data-v-f629f9be><strong data-v-f629f9be><span class="mobile-label-hidden" data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? unref(ui)("Temps") : unref(ui)("Modes et temps"))}</span><span class="mobile-label-only" data-v-f629f9be>${ssrInterpolate(unref(ui)("Temps"))}</span></strong>`);
          if (!unref(falcMode)) {
            _push(`<small data-v-f629f9be>${ssrInterpolate(unref(stepStatus).tenses ? unref(ui)(unref(stepStatus).tenses > 1 ? "{count} choisis" : "{count} choisi", { count: unref(stepStatus).tenses }) : unref(ui)("À choisir"))}</small>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</span></button><span class="wizard-steps__line" aria-hidden="true" data-v-f629f9be></span><button data-tour-wizard-step="3" class="${ssrRenderClass({
            "is-active": unref(currentStep) === 3,
            "is-complete": unref(currentStep) === 4,
            "tour-secondary-focus": unref(tourSecondaryWizardStep) === 3
          })}" type="button"${ssrIncludeBooleanAttr(!unref(isReady)) ? " disabled" : ""} data-v-f629f9be><span data-v-f629f9be>3</span><span data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(unref(ui)("Options"))}</strong>`);
          if (!unref(falcMode)) {
            _push(`<small data-v-f629f9be>${ssrInterpolate(unref(ui)("Finaliser le défi"))}</small>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</span></button>`);
          if (!unref(falcMode)) {
            _push(`<span class="wizard-steps__line" aria-hidden="true" data-v-f629f9be></span>`);
          } else {
            _push(`<!---->`);
          }
          if (!unref(falcMode)) {
            ...
            })}" type="button"${ssrIncludeBooleanAttr(!unref(isReady) || unref(isPreparingStep4)) ? " disabled" : ""} data-v-f629f9be><span data-v-f629f9be>4</span><span data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(props.startAtLaunch ? unref(ui)("Commencer") : unref(ui)("Créer"))}</strong><small data-v-f629f9be>${ssrInterpolate(unref(ui)("Utiliser le défi"))}</small></span></button>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(tourSecondaryWizardStep) !== null) {
            _push(`<span class="tour-wizard-step-indicator" style="${ssrRenderStyle(unref(tourWizardIndicatorStyle))}" aria-hidden="true" data-v-f629f9be></span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</nav>`);
        ...
        _push(`<div class="${ssrRenderClass([{ "wizard-content--home": unref(currentStep) === 0 }, "wizard-content"])}" data-v-f629f9be>`);
        if (unref(isPreparingStep4)) {
          _push(`<div class="wizard-step-preparing" role="status" aria-live="polite" data-v-f629f9be><span class="loader wizard-step-preparing__spinner" aria-hidden="true" data-v-f629f9be></span><strong data-v-f629f9be>${ssrInterpolate(unref(ui)("Préparation de ton défi…"))}</strong></div>`);
        } else if (unref(currentStep) === 0) {
          _push(`<div class="${ssrRenderClass([{ "wizard-home--falc": unref(falcMode) }, "wizard-home"])}" data-tour="home" data-v-f629f9be>`);
          if (unref(falcMode)) {
            _push(`<!--[-->`);
            if (unref(falcHomePanel) === null) {
              _push(`<div class="falc-home-actions" style="${ssrRenderStyle({ "display": "flex", "flex-direction": "column" })}" data-v-f629f9be><button class="falc-home-action" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("J’ai un code"))}</button><button class="falc-home-action" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("Choisir un défi"))}</button><button class="falc-home-action falc-home-action--primary" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("Créer mon exercice"))}</button></div>`);
            } else if (unref(falcHomePanel) === "code") {
              _push(`<div class="falc-home-panel code-loader" role="search"${ssrRenderAttr("aria-label", unref(ui)("Charger un défi avec son code"))} data-v-f629f9be><button class="falc-panel-back" type="button"${ssrRenderAttr("aria-label", unref(ui)("Retour"))} data-v-f629f9be>←</button><label id="wizard-falc-code-label" for="wizard-falc-code" data-v-f629f9be>${ssrInterpolate(unref(ui)("Écris le code du défi"))}</label><div class="code-loader__control" data-v-f629f9be><div id="wizard-falc-code" class="code-loader__code-entry" role="textbox" contenteditable="plaintext-only" aria-labelledby="wizard-falc-code-label" data-placeholder="AB-CD-EF-23"${ssrRenderAttr("aria-invalid", Boolean(unref(codeError)))} data-v-f629f9be></div><button class="primary-button" type="button"${ssrIncludeBooleanAttr(unref(busyAction) === "load") ? " disabled" : ""} data-v-f629f9be>${ssrInterpolate(unref(busyAction) === "load" ? unref(ui)("Chargement…") : unref(ui)("Ouvrir le défi"))}</button></div>`);
              if (unref(codeError)) {
                _push(`<p class="code-loader__error" role="alert" data-v-f629f9be>${ssrInterpolate(unref(codeError))}</p>`);
              } else {
                _push(`<!---->`);
              }
              _push(`</div>`);
            } else {
              _push(`<div class="falc-home-panel" data-v-f629f9be><button class="falc-panel-back" type="button"${ssrRenderAttr("aria-label", unref(ui)("Retour"))} data-v-f629f9be>←</button><h2 data-v-f629f9be>${ssrInterpolate(unref(ui)("Choisis un défi"))}</h2>`);
            ...
            _push(`<!--[--><div data-tour="code-loader" class="${ssrRenderClass([{ "is-arrival-highlighted": unref(highlightChallengeLoader) }, "code-loader"])}" role="search"${ssrRenderAttr("aria-label", unref(ui)("Charger un défi avec son code"))} data-v-f629f9be><div class="code-loader__heading" data-v-f629f9be><span class="code-loader__icon" aria-hidden="true" data-v-f629f9be><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-v-f629f9be><path d="M12 3v12" data-v-f629f9be></path><path d="m7 10 5 5 5-5" data-v-f629f9be></path><path d="M5 21h14" data-v-f629f9be></path></svg></span><div data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(unref(ui)("Tu as reçu un défi ?"))}</strong><small data-v-f629f9be>${ssrInterpolate(unref(ui)("Colle son code pour le reprendre immédiatement."))}</small></div></div><div class="code-loader__control" data-v-f629f9be><span id="wizard-challenge-code-label" class="sr-only" data-v-f629f9be>${ssrInterpolate(unref(ui)("Code du défi"))}</span><div id="wizard-challenge-code" class="code-loader__code-entry" role="textbox" contenteditable="plaintext-only" aria-labelledby="wizard-challenge-code-label" data-placeholder="AB-CD-EF-23"${ssrRenderAttr("aria-invalid", Boolean(unref(codeError)))} data-v-f629f9be></div><button class="primary-button wizard-home__outline-action" type="button"${ssrIncludeBooleanAttr(unref(catalogueStatus) !== "success" || unref(busyAction) === "load") ? " disabled" : ""} data-v-f629f9be>${ssrInterpolate(unref(busyAction) === "load" ? unref(ui)("Chargement…") : unref(ui)("Charger"))}</button></div>`);
            if (unref(codeError)) {
              _push(`<p class="code-loader__error" role="alert" data-v-f629f9be>${ssrInterpolate(unref(codeError))}</p>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div><div class="wizard-home__choices" data-v-f629f9be>`);
            if (!unref(presetExpanded)) {
              _push(`<button class="wizard-home__choice wizard-home__choice--preset is-collapsed" data-tour="presets" type="button" data-v-f629f9be><span class="wizard-home__choice-icon" aria-hidden="true" data-v-f629f9be>★</span><div data-v-f629f9be><h2 data-v-f629f9be>${ssrInterpolate(unref(ui)("Tu veux travailler un de nos défis ?"))}</h2></div><span class="secondary-button wizard-home__outline-action" aria-hidden="true" data-v-f629f9be>${ssrInterpolate(unref(ui)("Découvrir"))}</span></button>`);
            } else {
              _push(`<article data-tour="presets" class="${ssrRenderClass([{ "is-preset-selection": unref(presetStage) === "presets" }, "wizard-home__choice wizard-home__choice--preset"])}" data-v-f629f9be><span class="wizard-home__choice-icon" aria-hidden="true" data-v-f629f9be>★</span><div data-v-f629f9be><h2 data-v-f629f9be>${ssrInterpolate(unref(ui)("Tu veux travailler un de nos défis ?"))}</h2></div>`);
              _push(ssrRenderComponent(PresetPicker, {
            ...
            _push(`<article class="wizard-home__choice wizard-home__choice--custom" data-tour="build-custom" data-v-f629f9be><span class="wizard-home__choice-icon" aria-hidden="true" data-v-f629f9be>✎</span><div data-v-f629f9be><h2 data-v-f629f9be>${ssrInterpolate(unref(ui)("Tu veux construire ton propre défi ?"))}</h2><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Choisis les verbes, les modes, les temps et les options."))}</p></div><button class="${ssrRenderClass([{ "wizard-next-pulse": !unref(highlightChallengeLoader) }, "primary-button"])}" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("Construire un nouveau défi →"))}</button></article></div><div class="wizard-home__separator" aria-hidden="true" data-v-f629f9be></div><section class="wizard-home__seo-intro" aria-labelledby="home-features-title" data-v-f629f9be><header data-v-f629f9be><p class="wizard-home__seo-eyebrow" data-v-f629f9be>${ssrInterpolate(unref(ui)("Tout pour progresser"))}</p><h2 id="home-features-title" data-v-f629f9be>${ssrInterpolate(unref(ui)("Des exercices de conjugaison adaptés à tes besoins"))}</h2><p data-v-f629f9be>${ssrInterpolate(unref(ui)("TATITOTU propose des exercices de conjugaison française entièrement gratuits, interactifs et personnalisables, sans publicité."))}</p></header><div class="wizard-home__feature-grid" data-v-f629f9be><article data-v-f629f9be><h3 data-v-f629f9be>${ssrInterpolate(unref(ui)("Pour les élèves"))}</h3><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Choisis les verbes, les modes et les temps que tu souhaites travailler, personnalise les questions, puis commence ton entraînement."))}</p><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Les exercices peuvent être réalisés dans un format classique ou sous la forme d’un dialogue avec un coach virtuel qui t’aide pour chaque question."))}</p></article><article data-v-f629f9be><h3 data-v-f629f9be>${ssrInterpolate(unref(ui)("Pour les enseignants"))}</h3><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Tes propres exercices peuvent être partagés avec tes élèves."))}</p><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Les élèves peuvent aussi te partager leurs bilans pour un meilleur suivi."))}</p><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Tu peux aussi imprimer l’exercice en PDF ou DOCX avec corrigé."))}</p></article><article data-v-f629f9be><h3 data-v-f629f9be>${ssrInterpolate(unref(ui)("Des ressources utiles"))}</h3><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Pour apprendre et réviser, le site propose aussi des explications sur les modes et les temps."))}</p><p data-v-f629f9be>${ssrInterpolate(unref(ui)("Tu peux également consulter la conjugaison complète des verbes français, les règles d’accord du participe passé et les principales difficultés à éviter."))}</p></article></div></section><!--]-->`);
          }
          _push(`</div>`);
        } else if (unref(currentStep) === 1) {
          _push(`<div class="wizard-step wizard-step--selection" aria-labelledby="verbs-title" data-v-f629f9be><div class="wizard-step__actions wizard-step__actions--split" data-v-f629f9be><button class="secondary-button" type="button"${ssrRenderAttr("aria-label", unref(ui)("Étape précédente"))} data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? "←" : unref(ui)("← Nouveau défi"))}</button><div class="wizard-step__controls" data-v-f629f9be><button class="primary-button wizard-step__cta wizard-next-pulse" type="button"${ssrRenderAttr("aria-label", unref(ui)("Étape suivante"))}${ssrIncludeBooleanAttr(!unref(selectedVerbs).length) ? " disabled" : ""} data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? "→" : unref(ui)("Choisir les temps →"))}</button></div></div>`);
          if (unref(activePreset) && !unref(isPresetVerbEditing)) {
            _push(`<div class="wizard-step__intro wizard-step__intro--selection" data-v-f629f9be><h2 id="verbs-title" data-v-f629f9be>${ssrInterpolate(unref(ui)("Verbes du défi"))}</h2></div>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(activePreset) && !unref(isPresetVerbEditing)) {
            _push(`<section class="preset-verb-overview" data-v-f629f9be><header data-v-f629f9be><div data-v-f629f9be><p data-v-f629f9be>${ssrInterpolate(unref(selectedVerbs).length)} ${ssrInterpolate(unref(selectedVerbs).length === 1 ? unref(ui)("verbe") : unref(ui)("verbes"))} ${ssrInterpolate(unref(selectedVerbs).length === 1 ? unref(ui)("sélectionné") : unref(ui)("sélectionnés"))}</p><button class="preset-verb-overview__edit" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("Modifier la liste"))}</button></div></header><ul${ssrRenderAttrs({ name: "preset-verb" })} data-v-f629f9be>`);
            ssrRenderList(unref(displayedSelectedVerbs), (verb) => {
              _push(`<li data-v-f629f9be>${ssrInterpolate(verb.infinitif)}</li>`);
            });
            _push(`</ul></section>`);
          } else {
            _push(`<!--[--><div class="wizard-step__intro wizard-step__intro--selection" data-v-f629f9be><h2 id="verbs-title" data-v-f629f9be>${ssrInterpolate(unref(isPrefilledChallenge) ? unref(ui)("Verbes du défi") : unref(ui)("Choisis les verbes"))}</h2></div>`);
          ...
          _push(`<div class="wizard-step__bottom-actions" data-v-f629f9be><button class="primary-button wizard-step__cta wizard-next-pulse" type="button"${ssrRenderAttr("aria-label", unref(ui)("Étape suivante"))}${ssrIncludeBooleanAttr(!unref(selectedVerbs).length) ? " disabled" : ""} data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? "→" : unref(ui)("Choisir les temps →"))}</button></div></div>`);
        } else if (unref(currentStep) === 2) {
          _push(`<div class="wizard-step wizard-step--selection" aria-labelledby="tenses-title" data-v-f629f9be><div class="wizard-step__actions wizard-step__actions--split" data-v-f629f9be><button class="secondary-button" type="button"${ssrRenderAttr("aria-label", unref(ui)("Étape précédente"))} data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? "←" : unref(ui)("← Verbes"))}</button><div class="wizard-step__controls" data-v-f629f9be><button class="primary-button wizard-step__cta wizard-next-pulse" type="button"${ssrRenderAttr("aria-label", unref(ui)("Étape suivante"))}${ssrIncludeBooleanAttr(!unref(selectedTenses).length) ? " disabled" : ""} data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? "→" : unref(ui)("Choisir les options →"))}</button></div></div><div class="wizard-step__intro wizard-step__intro--selection" data-v-f629f9be><h2 data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? unref(ui)("Choisis les temps") : unref(isPrefilledChallenge) ? unref(ui)("Modes et temps") : unref(ui)("Choisis les modes et les temps"))}</h2></div>`);
          ...
          _push(`<div class="wizard-step wizard-review" data-v-f629f9be><div class="wizard-step__actions wizard-step__actions--split" data-v-f629f9be><button class="secondary-button" type="button"${ssrRenderAttr("aria-label", unref(ui)("Étape précédente"))} data-v-f629f9be> ← `);
          if (!unref(falcMode)) {
            _push(`<!--[--><span class="mobile-label-hidden" data-v-f629f9be>${ssrInterpolate(unref(ui)("Modes et temps"))}</span><span class="mobile-label-only" data-v-f629f9be>${ssrInterpolate(unref(ui)("Temps"))}</span><!--]-->`);
          } else {
            _push(`<!---->`);
          }
          _push(`</button><div class="wizard-step__controls" data-v-f629f9be><button class="primary-button wizard-step__cta wizard-step__cta--launch wizard-next-pulse" type="button" data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? unref(ui)("Commencer") : unref(ui)("Créer le défi"))}</button></div></div>`);
          if (!unref(falcMode)) {
            _push(`<div class="wizard-step__intro wizard-step__intro--selection" data-v-f629f9be><h2 data-v-f629f9be>${ssrInterpolate(unref(ui)("Options du défi"))}</h2></div>`);
          } else {
            _push(`<!---->`);
          }
          ...
          if (!unref(falcMode)) {
            _push(`<div class="wizard-step__bottom-actions" data-v-f629f9be><button class="primary-button wizard-step__cta wizard-step__cta--launch wizard-next-pulse" type="button" data-v-f629f9be>${ssrInterpolate(unref(falcMode) ? unref(ui)("Commencer") : unref(ui)("Créer le défi"))}</button></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        } else {
          _push(`<div class="wizard-step wizard-launch-step" data-v-f629f9be>`);
          if (!props.startAtLaunch) {
            _push(`<div class="wizard-step__actions wizard-step__actions--split" data-v-f629f9be><button class="secondary-button" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("← Options"))}</button></div>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(showLaunchSummary) || unref(showSavedChallengeSummary)) {
            _push(`<section class="launch-summary"${ssrRenderAttr("aria-labelledby", __props.launchTitle || unref(activePreset) || unref(savedChallengeTitle) ? "launch-challenge-title" : void 0)} data-v-f629f9be><div class="${ssrRenderClass([{ "launch-summary__heading--public": __props.launchTitle }, "launch-summary__heading"])}" data-v-f629f9be><div data-v-f629f9be>`);
            if (__props.launchCategory || unref(activePreset)) {
              _push(`<p class="builder-card__eyebrow" data-v-f629f9be>${ssrInterpolate(__props.launchCategory || unref(activePresetGroupLabel))}</p>`);
            } else {
              _push(`<!---->`);
            }
            if (__props.launchTitle) {
              _push(`<h1 id="launch-challenge-title" data-v-f629f9be>${ssrInterpolate(__props.launchTitle)}</h1>`);
            } else if (unref(activePreset) || unref(savedChallengeTitle)) {
              _push(`<h2 id="launch-challenge-title" data-v-f629f9be>${ssrInterpolate(unref(activePreset)?.label || unref(savedChallengeTitle))}</h2>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div><div class="launch-summary__counts" data-v-f629f9be><span data-v-f629f9be>${ssrInterpolate(unref(ui)(unref(selectedVerbs).length > 1 ? "{count} verbes" : "{count} verbe", { count: unref(selectedVerbs).length }))}</span><span data-v-f629f9be>${ssrInterpolate(unref(ui)("{count} temps", { count: unref(selectedTenses).length }))}</span></div></div>`);
            if (__props.launchDescription || unref(activePreset)?.description || unref(savedChallengeDescription)) {
              _push(`<p class="launch-summary__description" data-v-f629f9be>${ssrInterpolate(__props.launchDescription || unref(activePreset)?.description || unref(savedChallengeDescription))}</p>`);
            } else {
              _push(`<!---->`);
            }
            if (__props.launchEditHint) {
              _push(`<p class="launch-summary__edit-hint" data-v-f629f9be><span data-v-f629f9be>${ssrInterpolate(__props.launchEditHint.before)}</span><button class="launch-summary__step launch-summary__step--verbs" type="button"${ssrRenderAttr("aria-label", __props.launchEditHint.verbsLabel)} data-v-f629f9be>1</button><span data-v-f629f9be>${ssrInterpolate(__props.launchEditHint.between)}</span><button class="launch-summary__step launch-summary__step--tenses" type="button"${ssrRenderAttr("aria-label", __props.launchEditHint.tensesLabel)} data-v-f629f9be>2</button><span data-v-f629f9be>${ssrInterpolate(__props.launchEditHint.after)}</span></p>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</section>`);
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(isTourWelcomeOpen) && !unref(guidedTourDisabled)) {
          _push2(`<div class="tour-welcome-backdrop" data-v-f629f9be><section class="tour-welcome-dialog" role="dialog" aria-modal="true" aria-labelledby="tour-welcome-title" data-v-f629f9be><div class="tour-welcome-dialog__languages" role="group"${ssrRenderAttr("aria-label", unref(ui)("Langue de l’interface"))} data-v-f629f9be><!--[-->`);
          ssrRenderList(unref(tourLanguageOptions), (option) => {
            _push2(`<button type="button" class="${ssrRenderClass({ "is-active": unref(interfaceLocale) === option.value })}"${ssrRenderAttr("aria-label", option.label)}${ssrRenderAttr("aria-pressed", unref(interfaceLocale) === option.value)}${ssrRenderAttr("title", option.label)} data-v-f629f9be><span aria-hidden="true" data-v-f629f9be>${ssrInterpolate(option.flag)}</span></button>`);
          });
          _push2(`<!--]--></div><button class="tour-welcome-dialog__close" type="button"${ssrRenderAttr("aria-label", unref(ui)("Fermer"))} data-v-f629f9be>×</button><span class="tour-welcome-dialog__icon" aria-hidden="true" data-v-f629f9be>?</span><h2 id="tour-welcome-title" data-v-f629f9be>${ssrInterpolate(unref(tourCopy).welcomeTitle)}</h2><p data-v-f629f9be>${ssrInterpolate(unref(tourCopy).welcomeBody)}</p><div class="tour-welcome-dialog__choices" data-v-f629f9be><button type="button" data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(unref(tourCopy).quickTitle)}</strong><small data-v-f629f9be>${ssrInterpolate(unref(tourCopy).quickMeta)}</small></button><button type="button" data-v-f629f9be><strong data-v-f629f9be>${ssrInterpolate(unref(tourCopy).fullTitle)}</strong><small data-v-f629f9be>${ssrInterpolate(unref(tourCopy).fullMeta)}</small></button></div><button class="tour-welcome-dialog__later" type="button" data-v-f629f9be>${ssrInterpolate(unref(ui)("Fermer"))}</button></section></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
...
const WizardChallengeWorkspace = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main, [["__scopeId", "data-v-f629f9be"]]), { __name: "ChallengeWizardChallengeWorkspace" });

export { WizardChallengeWorkspace as W };
//# sourceMappingURL=WizardChallengeWorkspace-GcGKeznV.mjs.map
