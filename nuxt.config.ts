...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    contactFromEmail: process.env.NUXT_CONTACT_FROM_EMAIL || process.env.CONTACT_FROM_EMAIL,
    smtpHost: process.env.NUXT_SMTP_HOST || process.env.SMTP_HOST,
    smtpPort: Number(process.env.NUXT_SMTP_PORT || process.env.SMTP_PORT || 587),
    smtpSecure: (process.env.NUXT_SMTP_SECURE || process.env.SMTP_SECURE || '').toLocaleLowerCase() === 'true',
    smtpUser: process.env.NUXT_SMTP_USER || process.env.SMTP_USER,
    smtpPassword: process.env.NUXT_SMTP_PASSWORD || process.env.SMTP_PASSWORD,
