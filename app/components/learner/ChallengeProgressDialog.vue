...
const errorMaximum = computed(() => Math.max(1, ...points.value.map(point => point.incorrectCount)))
...
const errorCoordinates = computed(() => chartCoordinates(point => point.incorrectCount, errorMaximum.value))
const successPolyline = computed(() => successCoordinates.value.map(point => `${point.x},${point.y}`).join(' '))
const errorPolyline = computed(() => errorCoordinates.value.map(point => `${point.x},${point.y}`).join(' '))
...
function trendDelta(trend: ProgressTrend, unit: 'points' | 'errors') {
  if (trend.direction === 'insufficient') return 'Il faut au moins deux séances avec des réponses.'
  const sign = trend.delta > 0 ? '+' : ''
  if (unit === 'points') return `${sign}${trend.delta} point${Math.abs(trend.delta) > 1 ? 's' : ''}`
  return `${sign}${trend.delta} erreur${Math.abs(trend.delta) > 1 ? 's' : ''}`
}

function trendTone(trend: ProgressTrend, metric: 'success' | 'errors') {
  if (trend.direction === 'stable' || trend.direction === 'insufficient') return 'neutral'
  const improving = metric === 'success' ? trend.direction === 'up' : trend.direction === 'down'
    ...
    <div class="challenge-progress-overlay" @click.self="emit('close')">
      ...
      >
        <header class="challenge-progress-dialog__header">
          <div>
            <p>Analyse du défi</p>
            <h2 id="challenge-progress-title">Progression · {{ challengeLabel }}</h2>
          </div>
        ...
        <div v-else-if="summary && points.length" class="challenge-progress-content">
          <p class="challenge-progress-count">
            Analyse fondée sur <strong>{{ points.length }} séance{{ points.length > 1 ? 's' : '' }} avec des réponses</strong>.
          </p>

          <div class="challenge-progress-trends">
            <article :class="`is-${trendTone(summary.successTrend, 'success')}`">
              <span>Pourcentage de réussite</span>
              <strong>{{ trendLabel(summary.successTrend) }}</strong>
              <small>{{ trendDelta(summary.successTrend, 'points') }}</small>
            </article>
            <article :class="`is-${trendTone(summary.errorTrend, 'errors')}`">
              <span>Nombre d’erreurs</span>
              <strong>{{ trendLabel(summary.errorTrend) }}</strong>
              <small>{{ trendDelta(summary.errorTrend, 'errors') }}</small>
            </article>
          </div>
          ...
          <div class="challenge-progress-chart challenge-progress-chart--errors">
            <header>
              <div>
                <span>Erreurs</span>
                <strong>{{ points.at(-1)?.incorrectCount }}</strong>
              </div>
              <small>Une baisse est un progrès.</small>
            </header>
            <svg :viewBox="`0 0 ${chartWidth} ${chartHeight}`" role="img" aria-label="Évolution du nombre d’erreurs">
              <line v-for="level in [0, .5, 1]" :key="level" :x1="chartLeft" :x2="chartWidth - chartRight" :y1="chartTop + (1 - level) * (chartHeight - chartTop - chartBottom)" :y2="chartTop + (1 - level) * (chartHeight - chartTop - chartBottom)" />
              <text x="4" :y="chartTop + 4">{{ errorMaximum }}</text>
              <text x="4" :y="chartHeight - chartBottom + 4">0</text>
              <polyline v-if="errorCoordinates.length > 1" :points="errorPolyline" />
              <circle v-for="coordinate in errorCoordinates" :key="coordinate.point.id" :cx="coordinate.x" :cy="coordinate.y" r="6">
                <title>{{ formattedDate(coordinate.point.occurredAt) }} : {{ coordinate.point.incorrectCount }} erreur{{ coordinate.point.incorrectCount > 1 ? 's' : '' }}</title>
              </circle>
              <text :x="chartLeft" :y="chartHeight - 5">{{ formattedDate(firstDate) }}</text>
              <text v-if="lastDate !== firstDate" :x="chartWidth - chartRight" :y="chartHeight - 5" text-anchor="end">{{ formattedDate(lastDate) }}</text>
            </svg>
