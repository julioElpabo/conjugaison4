import { w as withoutIndicativeMode, l as literaryIdentificationCoachHelpBlocks, v as visibleCoachHelpBlocks, d as areOnlyIndicativeTenses, c as coachHelpQuestionVariables, e as localizedCoachVerbDefinition, s as sanitizeCoachHtml, _ as __nuxt_component_0 } from './CoachHelpPanel-BQHM-KrB.mjs';
import { defineComponent, computed, ref, shallowRef, watch, markRaw, reactive, useTemplateRef, unref, createVNode, resolveDynamicComponent, nextTick, useSSRContext } from 'vue';
...
import { aW as SUBJECT_PRONOUN_PLACEHOLDER, aI as conjugationRequiresSubjectPronoun, aK as providedSubjunctiveInputPrefix, aJ as conjugationAnswerPlaceholder, a9 as learnerErrorDetailText } from '../nitro/nitro.mjs';
...
function coachReactionText(text, requiredText = "", fallbackText = "", valueAlreadyCovered = "") {
  const primaryText = text.trim() || fallbackText.trim();
  const visiblePrimaryText = primaryText.replace(/<[^>]*>/gu, " ").replace(/\s+/gu, " ").trim();
  const effectiveRequiredText = valueAlreadyCovered.trim() && visiblePrimaryText.includes(valueAlreadyCovered.trim()) ? "" : requiredText.trim();
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  props: {
    ...
    canSaveChallenge: { type: Boolean },
    analyticsMetadata: {}
  },
  emits: ["close", "changeCoach", "saveChallenge"],
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    watch(printSummaryOpen, async (open) => {
      ...
      printSummaryComponent.value = markRaw((await import('./ExerciseSummaryPrintPreview-Dlup2Jj7.mjs')).default);
    });
    ...
    const failedQuestionIndexes = reactive(/* @__PURE__ */ new Set());
    ...
    const helpAnswersRevealed = computed(() => selectedCoachHelpProfile.value.revealsAnswers && failedQuestionIndexes.has(helpQuestionIndex.value ?? currentIndex.value));
    const effectiveHelpApproach = computed(() => selectedCoachHelpProfile.value.revealsAnswers && !helpAnswersRevealed.value ? "complete" : props.coach.helpApproach);
    const usesAllophoneHelp = computed(() => effectiveHelpApproach.value === "allophone" && !usesIdentificationHelp.value);
    computed(() => helpAnswersRevealed.value && (props.coach.helpApproach === "complete-avec-reponses" || props.coach.helpApproach === "allophone"));
    const usesDelayedAnswerAudio = computed(() => helpAnswersRevealed.value && props.coach.helpApproach === "tres-condensee");
    ...
    const helpBlocks = computed(() => usesIdentificationHelp.value ? literaryIdentificationCoachHelpBlocks() : visibleCoachHelpBlocks(effectiveHelpApproach.value, helpQuestion.value));
    function displayedChatAnswer(question, expectedAnswer) {
    ...
    const revealedHelpAnswers = computed(() => helpAnswersRevealed.value ? [...new Set(helpQuestion.value?.reponsesPourCorrige.map((item) => displayedChatAnswer(helpQuestion.value, item)).filter(Boolean) || [])] : []);
    ...
    const attemptSummaries = computed(() => attempts.value.map((attempt, index) => {
      ...
      return {
        ...
        expectedAnswer: (attempt.question.reponsesPourCorrige.length ? attempt.question.reponsesPourCorrige : attempt.question.reponses).map((answer2) => displayedChatAnswer(attempt.question, answer2)).join(` ${ui("ou")} `),
        ...
        identificationForm: (attempt.question.exerciseKind ? attempt.question.exerciseKind !== "conjugation" : isIdentificationExercise.value) && attempt.status === "incorrect" ? identificationFormParts(attempt.question) : null,
    ...
    function markProgrammaticThreadScroll() {
      if (programmaticScrollTimer !== null) (void 0).clearTimeout(programmaticScrollTimer);
      programmaticScrollTimer = (void 0).setTimeout(() => {
    ...
    function contextFor(question, hideIdentificationAnswer = false, keepGrammarFrench = false) {
      ...
      const mayRevealAnswer = questionIndex >= 0 && failedQuestionIndexes.has(questionIndex);
      const correctedAnswers = question?.reponsesPourCorrige.filter((value) => value.trim()) || [];
      const expectedAnswers = correctedAnswers.length ? correctedAnswers : question?.reponses.filter((value) => value.trim()) || [];
      return {
        ...
        participle: mayRevealAnswer ? reminder?.participle : void 0,
        ...
        expectedAnswer: hidesAnswer || !mayRevealAnswer || !expectedAnswers.length || !question ? void 0 : expectedAnswers.map((answer2) => displayedChatAnswer(question, answer2)).join(" ou "),
    ...
    async function addCoachReaction(eventType, context, tone, requiredText = "", fallbackText = "") {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        _push2(`<div class="chat-overlay" data-tour="chat-exercise" style="${ssrRenderStyle(unref(tourDemoReady) ? null : { display: "none" })}" data-v-c0200226><div class="${ssrRenderClass([{ "chat-dialogs--with-help": unref(helpOpen), "chat-dialogs--confirming": unref(closeConfirmationOpen) }, "chat-dialogs"])}" style="${ssrRenderStyle(unref(coachChatStyle))}" role="dialog" aria-modal="true" aria-labelledby="chat-title" tabindex="-1" data-v-c0200226><section class="chat-dialog" data-tour="chat-dialog" role="region" aria-labelledby="chat-title" data-v-c0200226><header class="chat-header" data-v-c0200226><img class="coach-avatar"${ssrRenderAttr("src", __props.coach.avatarPath)} alt="" data-v-c0200226><div class="chat-header__identity" data-v-c0200226><h2 id="chat-title" data-v-c0200226>${ssrInterpolate(__props.coach.firstName)}</h2>`);
        if (__props.coach.likes) {
          _push2(`<p class="chat-header__likes" data-v-c0200226><strong data-v-c0200226>${ssrInterpolate(unref(ui)("Aime :"))}</strong> ${ssrInterpolate(__props.coach.likes)}</p>`);
        } else {
          _push2(`<!---->`);
        }
        _push2(`</div><div class="chat-header__actions" data-v-c0200226><button type="button" class="chat-close"${ssrRenderAttr("aria-label", unref(ui)("Quitter le chat"))} data-v-c0200226>×</button></div></header><div class="chat-progress"${ssrRenderAttr("aria-label", unref(ui)("Progression"))} data-v-c0200226><span style="${ssrRenderStyle({ width: `${unref(finished) ? 100 : (unref(questionNumberOffset) + unref(currentIndex)) / unref(displayedQuestionCount) * 100}%` })}" data-v-c0200226></span></div>`);
        if (!unref(finished) && unref(currentQuestion)) {
          _push2(`<div class="chat-instruction" data-v-c0200226><span data-v-c0200226>${ssrInterpolate(unref(ui)("Question {current} sur {total}", { current: unref(displayedQuestionNumber), total: unref(displayedQuestionCount) }))}</span></div>`);
        } else {
          _push2(`<!---->`);
        }
        _push2(`<div class="chat-thread" aria-live="polite" data-v-c0200226><!--[-->`);
        ssrRenderList(unref(visibleMessages), (message) => {
          _push2(`<div class="${ssrRenderClass([`chat-message-row--${message.author}`, "chat-message-row"])}" data-v-c0200226><div${ssrRenderAttr("data-chat-message-id", message.id)} class="${ssrRenderClass([[
          ...
          ], "chat-message"])}"${ssrRenderAttr("role", message.author === "learner" && message.questionIndex !== void 0 ? "button" : void 0)}${ssrRenderAttr("tabindex", message.author === "learner" && message.questionIndex !== void 0 ? 0 : void 0)}${ssrRenderAttr("aria-label", message.author === "learner" && message.questionIndex !== void 0 ? unref(ui)("Voir l’aide de la question {number} pour la réponse {answer}", { number: message.questionIndex + 1, answer: message.text }) : void 0)} data-v-c0200226>`);
          if (message.errorDetails?.length) {
            _push2(ssrRenderComponent(LearnerErrorFeedback, {
              details: message.errorDetails
            }, null, _parent));
          } else if (message.literaryCitation) {
            _push2(`<div class="chat-literary-question" data-v-c0200226><blockquote class="chat-literary-citation" data-v-c0200226><p data-v-c0200226><span data-v-c0200226>${ssrInterpolate(message.literaryCitation.before)}</span><mark data-v-c0200226>${ssrInterpolate(message.literaryCitation.target)}</mark><span data-v-c0200226>${ssrInterpolate(message.literaryCitation.after)}</span></p><footer data-v-c0200226>${ssrInterpolate(message.literaryCitation.author)}, <cite data-v-c0200226>${ssrInterpolate(message.literaryCitation.work)}</cite></footer></blockquote></div>`);
          } else if (message.identificationForm) {
            _push2(`<p class="chat-identification-form" data-v-c0200226><span data-v-c0200226>${ssrInterpolate(message.identificationForm.before)}</span><mark data-v-c0200226>${ssrInterpolate(message.identificationForm.target)}</mark><span data-v-c0200226>${ssrInterpolate(message.identificationForm.after)}</span></p>`);
          } else if (message.answerComparison) {
            _push2(`<div class="answer-comparison" data-v-c0200226><strong data-v-c0200226>${ssrInterpolate(message.answerComparison.mode === "focused" ? unref(ui)("Regarde où ça change :") : unref(ui)("Repars de la correction complète :"))}</strong><div class="answer-comparison__line answer-comparison__line--learner" data-v-c0200226><small data-v-c0200226>${ssrInterpolate(unref(ui)("Ta réponse"))}</small><p data-v-c0200226><!--[-->`);
            ssrRenderList(message.answerComparison.learnerParts, (part, partIndex) => {
              _push2(`<span class="${ssrRenderClass(`answer-comparison__part--${part.kind}`)}" data-v-c0200226>${ssrInterpolate(part.text)}</span>`);
            });
            _push2(`<!--]--></p></div><div class="answer-comparison__line answer-comparison__line--expected" data-v-c0200226><small data-v-c0200226>${ssrInterpolate(unref(ui)("Correction"))}</small><p data-v-c0200226><!--[-->`);
            ssrRenderList(message.answerComparison.expectedParts, (part, partIndex) => {
              _push2(`<span class="${ssrRenderClass(`answer-comparison__part--${part.kind}`)}" data-v-c0200226>${ssrInterpolate(part.text)}</span>`);
            });
            _push2(`<!--]--></p></div>`);
            if (message.answerComparison.mode === "full") {
              _push2(`<small class="answer-comparison__guidance" data-v-c0200226>${ssrInterpolate(unref(ui)("Les deux réponses sont très différentes : observe d’abord la construction complète."))}</small>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div>`);
          } else if (message.text && message.author === "coach" && message.answerLine) {
            _push2(`<span class="chat-message__text chat-message__text--emphasis" data-v-c0200226><!--[-->`);
            ssrRenderList(answerLineParts(message.text), (part, partIndex) => {
              _push2(`<span class="${ssrRenderClass({ "chat-answer-line": part.isLine })}" data-v-c0200226>${ssrInterpolate(part.text)}</span>`);
            });
            _push2(`<!--]--></span>`);
          } else if (message.text && message.author === "coach") {
            _push2(`<span class="${ssrRenderClass([{ "chat-message__text--emphasis": message.emphasis }, "chat-message__text"])}" data-v-c0200226>${unref(sanitizeCoachHtml)(message.text) ?? ""}</span>`);
          } else if (message.text && message.emphasis) {
            _push2(`<strong data-v-c0200226>${ssrInterpolate(message.text)}</strong>`);
          } else if (message.text) {
            _push2(`<span data-v-c0200226>${ssrInterpolate(message.text)}</span>`);
          } else {
            _push2(`<!---->`);
          }
          if (message.consultVerbId) {
            _push2(`<button type="button" class="chat-consult-verb-link" data-v-c0200226>${ssrInterpolate(unref(ui)("Consulter le verbe"))}</button>`);
          } else {
            _push2(`<!---->`);
          }
          if (unref(audioReadingEnabled) && message.spokenAnswer && unref(speechSupported)) {
            _push2(`<button type="button" class="${ssrRenderClass([{ "chat-hear-answer-button--icon-only": message.speechOnly }, "chat-hear-answer-button"])}"${ssrRenderAttr("aria-label", unref(speakingMessageId) === message.id ? unref(ui)("Arrêter la lecture") : unref(ui)("Entendre la réponse"))}${ssrRenderAttr("aria-pressed", unref(speakingMessageId) === message.id)} data-v-c0200226>`);
            ...
            if (!message.speechOnly) {
              _push2(`<span data-v-c0200226>${ssrInterpolate(unref(speakingMessageId) === message.id ? unref(ui)("Arrêter la lecture") : unref(ui)("Entendre la réponse"))}</span>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</button>`);
          ...
          if (message.media?.mediaType === "video") {
            _push2(`<video${ssrRenderAttr("src", message.media.filePath)}${ssrRenderAttr("aria-label", message.media.altText)} muted playsinline controls data-v-c0200226></video>`);
          } else if (message.media) {
            _push2(`<img class="${ssrRenderClass({ "chat-media--emoji": message.media.mediaType === "emoji" })}"${ssrRenderAttr("src", message.media.filePath)}${ssrRenderAttr("alt", message.media.altText)} data-v-c0200226>`);
          } else {
            _push2(`<!---->`);
          }
          if (message.identificationPrompt && message.questionIndex === unref(currentIndex)) {
            _push2(`<div class="chat-identification-choices" data-v-c0200226>`);
            if (unref(activeExerciseKind) === "tense-identification" && unref(selectedIdentificationMode)) {
              _push2(`<div class="chat-tense-choice-step" data-v-c0200226><div class="chat-tense-choice-step__header" data-v-c0200226><button type="button" data-v-c0200226>← ${ssrInterpolate(unref(ui)("Modes"))}</button><strong data-v-c0200226>${ssrInterpolate(unref(ui)("Choisis le temps"))}</strong></div><div class="chat-tense-choices" role="group"${ssrRenderAttr("aria-label", unref(ui)("Choisis le temps"))} data-v-c0200226><!--[-->`);
              ssrRenderList(unref(selectedModeTenseRows), (row) => {
                _push2(`<div class="chat-tense-choice-row" data-v-c0200226>`);
                if (row.simple) {
                  _push2(`<button type="button"${ssrIncludeBooleanAttr(unref(waitingForNext) || unref(posingQuestion) || unref(deliveringFeedback)) ? " disabled" : ""} data-v-c0200226>${ssrInterpolate(unref(uiLabel)(row.simple.name))}</button>`);
                } else {
                  _push2(`<span aria-hidden="true" data-v-c0200226></span>`);
                }
                if (row.compound) {
                  _push2(`<button type="button"${ssrIncludeBooleanAttr(unref(waitingForNext) || unref(posingQuestion) || unref(deliveringFeedback)) ? " disabled" : ""} data-v-c0200226>${ssrInterpolate(unref(uiLabel)(row.compound.name))}</button>`);
                } else {
                  _push2(`<span aria-hidden="true" data-v-c0200226></span>`);
                }
                _push2(`</div>`);
              ...
              _push2(`<div class="chat-mode-choices" role="group"${ssrRenderAttr("aria-label", unref(ui)("Choisis le mode"))} data-v-c0200226><!--[-->`);
              ssrRenderList(unref(displayedIdentificationModeChoices), (choice) => {
                _push2(`<button type="button"${ssrIncludeBooleanAttr(unref(waitingForNext) || unref(posingQuestion) || unref(deliveringFeedback)) ? " disabled" : ""} data-v-c0200226>${ssrInterpolate(choice.label)}</button>`);
              });
              _push2(`<!--]--></div>`);
            }
            _push2(`</div>`);
        ...
        if (unref(finalSummaryPreparing)) {
          _push2(`<div class="chat-summary-loading" role="status" aria-live="polite" data-v-c0200226><span aria-hidden="true" data-v-c0200226></span><strong data-v-c0200226>${ssrInterpolate(unref(ui)("Création du bilan"))}</strong></div>`);
        } else {
          _push2(`<!---->`);
        }
        if (unref(finalSummaryVisible)) {
          _push2(`<section class="chat-summary-tool" aria-labelledby="chat-summary-title" data-v-c0200226><header data-v-c0200226><div data-v-c0200226><h3 id="chat-summary-title" data-v-c0200226>${ssrInterpolate(unref(ui)("Bilan du défi"))}</h3></div><strong data-v-c0200226>${ssrInterpolate(unref(score))} %</strong></header><ol class="chat-summary-list" data-v-c0200226><!--[-->`);
          ssrRenderList(unref(attemptSummaries), (item) => {
            _push2(`<li class="${ssrRenderClass([`is-${item.answerWasHeard ? "heard" : item.status}`, { "is-help-selected": unref(helpOpen) && item.questionIndex === unref(helpQuestionIndex) }])}" style="${ssrRenderStyle({ "--summary-item-index": `${item.index - 1}` })}" role="button" tabindex="0"${ssrRenderAttr("aria-label", unref(ui)("Voir l’aide de la question {number} : {question}", { number: item.index, question: item.questionLabel }))} data-v-c0200226><span class="chat-summary-list__status" aria-hidden="true" data-v-c0200226>${ssrInterpolate(item.answerWasHeard ? "🔊" : item.status === "correct" ? "✓" : "×")}</span><div data-v-c0200226><div class="chat-summary-question-row" data-v-c0200226><strong class="chat-summary-list__question" data-v-c0200226><span data-v-c0200226>${ssrInterpolate(unref(ui)("Question"))} ${ssrInterpolate(item.index)}</span><span data-v-c0200226>${ssrInterpolate(item.questionLabel)}</span></strong>`);
            if (item.verbId) {
              _push2(`<button type="button" class="chat-summary-consult-link" data-v-c0200226>${ssrInterpolate(unref(ui)("Consulter"))}</button>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div>`);
            if (item.identificationForm) {
              _push2(`<blockquote class="chat-summary-list__citation" data-v-c0200226><p data-v-c0200226><span data-v-c0200226>${ssrInterpolate(item.identificationForm.before)}</span><mark data-v-c0200226>${ssrInterpolate(item.identificationForm.target)}</mark><span data-v-c0200226>${ssrInterpolate(item.identificationForm.after)}</span></p>`);
              if (item.literaryCitation) {
                _push2(`<footer data-v-c0200226>${ssrInterpolate(item.literaryCitation.author)}, <cite data-v-c0200226>${ssrInterpolate(item.literaryCitation.work)}</cite></footer>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</blockquote>`);
            ...
            _push2(`<dl data-v-c0200226><div data-v-c0200226><dt data-v-c0200226>${ssrInterpolate(unref(ui)("Réponse donnée"))}</dt><dd data-v-c0200226>${ssrInterpolate(item.learnerAnswer)}</dd></div><div data-v-c0200226><dt data-v-c0200226>${ssrInterpolate(unref(ui)("Bonne réponse"))}</dt><dd data-v-c0200226>${ssrInterpolate(item.expectedAnswer)}</dd></div></dl></div></li>`);
          });
          _push2(`<!--]--></ol><footer data-v-c0200226><strong data-v-c0200226>${ssrInterpolate(unref(correctCount))} / ${ssrInterpolate(unref(attempts).length)}</strong><span data-v-c0200226>${ssrInterpolate(unref(correctCount) === 1 ? unref(ui)("réponse juste") : unref(ui)("réponses justes"))}</span></footer></section>`);
        } else {
          _push2(`<!---->`);
        }
        if (unref(finalSummaryVisible)) {
          _push2(`<div class="chat-message-row chat-message-row--coach" data-v-c0200226><div class="chat-message chat-message--coach chat-restart-prompt" data-v-c0200226><span data-v-c0200226>${ssrInterpolate(unref(restartCoachMessage).text)}</span><div class="chat-restart-prompt__actions" data-v-c0200226><button type="button" class="chat-restart-prompt__same"${ssrIncludeBooleanAttr(unref(regeneratingQuestions)) ? " disabled" : ""} data-v-c0200226><span aria-hidden="true" data-v-c0200226>↻</span>${ssrInterpolate(unref(ui)("Avec les mêmes questions"))}</button><button type="button" class="chat-restart-prompt__new"${ssrIncludeBooleanAttr(unref(regeneratingQuestions)) ? " disabled" : ""} data-v-c0200226><span aria-hidden="true" data-v-c0200226>↻</span>${ssrInterpolate(unref(regeneratingQuestions) ? unref(ui)("Préparation…") : unref(ui)("Avec d’autres questions"))}</button>`);
          if (__props.canSaveChallenge) {
            _push2(`<button type="button" class="chat-restart-prompt__challenge-share"${ssrIncludeBooleanAttr(unref(regeneratingQuestions)) ? " disabled" : ""} data-v-c0200226><span aria-hidden="true" data-v-c0200226>`);
            _push2(ssrRenderComponent(unref(FontAwesomeIcon), { icon: unref(faArrowUpFromBracket) }, null, _parent));
            _push2(`</span>${ssrInterpolate(unref(ui)("Enregistrer / partager ce défi"))}</button>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`<button type="button" class="chat-restart-prompt__share"${ssrIncludeBooleanAttr(unref(regeneratingQuestions)) ? " disabled" : ""} data-v-c0200226><span aria-hidden="true" data-v-c0200226>`);
          _push2(ssrRenderComponent(unref(FontAwesomeIcon), { icon: unref(faArrowUpFromBracket) }, null, _parent));
          _push2(`</span>${ssrInterpolate(unref(ui)("Partager mon bilan"))}</button><button type="button" class="chat-restart-prompt__print"${ssrIncludeBooleanAttr(unref(regeneratingQuestions)) ? " disabled" : ""} data-v-c0200226><span aria-hidden="true" data-v-c0200226>`);
          _push2(ssrRenderComponent(unref(FontAwesomeIcon), { icon: unref(faPrint) }, null, _parent));
          _push2(`</span>${ssrInterpolate(unref(ui)("Imprimer mon bilan"))}</button><button type="button" class="chat-restart-prompt__quit"${ssrIncludeBooleanAttr(unref(regeneratingQuestions)) ? " disabled" : ""} data-v-c0200226>${ssrInterpolate(unref(ui)("Quitter le chat"))}</button></div>`);
          if (unref(restartError)) {
            _push2(`<small class="chat-restart-prompt__error" role="alert" data-v-c0200226>${ssrInterpolate(unref(restartError))}</small>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`</div>`);
          if (unref(coachMessageAudioEnabled)) {
            _push2(`<button type="button" class="${ssrRenderClass([{ "is-error": unref(coachAudioErrorMessageId) === unref(restartCoachMessage).id }, "chat-message-audio-button"])}"${ssrRenderAttr("aria-label", unref(coachAudioPlayingMessageId) === unref(restartCoachMessage).id || unref(coachAudioLoadingMessageId) === unref(restartCoachMessage).id ? unref(ui)("Arrêter la lecture") : unref(ui)("Écouter le message du coach"))}${ssrRenderAttr("aria-pressed", unref(coachAudioPlayingMessageId) === unref(restartCoachMessage).id)}${ssrRenderAttr("title", unref(coachAudioErrorMessageId) === unref(restartCoachMessage).id ? unref(ui)("La lecture audio a échoué. Réessayer.") : void 0)} data-v-c0200226>`);
            _push2(ssrRenderComponent(unref(FontAwesomeIcon), {
        ...
        if (unref(helpOpen) && (unref(targetedHelp) || unref(usesIdentificationHelp))) {
          _push2(ssrRenderComponent(_component_CoachHelpPanel, {
            ...
            "revealed-answers": unref(revealedHelpAnswers),
...
const ChatExercise = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main, [["__scopeId", "data-v-c0200226"]]), { __name: "ExerciseChatExercise" });

export { ChatExercise as default };
//# sourceMappingURL=ChatExercise-C-57oFcT.mjs.map
