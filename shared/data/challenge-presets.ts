...
export const challengePresetDefinitions = [
  ...
  { id: 'groupe3ir', label: 'Troisième groupe en -ir', description: 'Verbes du troisième groupe terminés par -ir.', group: 'verb-group', criteria: [{ field: 'groupeConjugaison', operator: 'equals', value: 3 }, { field: 'terminaison', operator: 'equals', value: 'ir' }], tenseIds: personalPresent, questionCount: 10 },
  { id: 'groupe3oir', label: 'Troisième groupe en -oir', description: 'Verbes du troisième groupe terminés par -oir.', group: 'verb-group', criteria: [{ field: 'groupeConjugaison', operator: 'equals', value: 3 }, { field: 'terminaison', operator: 'equals', value: 'oir' }], tenseIds: personalPresent, questionCount: 10 },
  { id: 'groupe3autres', label: 'Autres verbes du troisième groupe', description: 'Verbes du troisième groupe hors terminaisons -ir et -oir.', group: 'verb-group', criteria: [{ field: 'groupeConjugaison', operator: 'equals', value: 3 }, { field: 'terminaison', operator: 'not-in', value: ['ir', 'oir'] }], tenseIds: personalPresent, questionCount: 10 },
  { id: 'ger', label: 'Verbes en -ger', description: 'Particularités orthographiques des verbes en -ger.', group: 'spelling', criteria: [{ field: 'particularites', operator: 'includes', value: 'ger' }], tenseIds: coreTenses, questionCount: 10 },
  { id: 'cer', label: 'Verbes en -cer', description: 'Particularités orthographiques des verbes en -cer.', group: 'spelling', criteria: [{ field: 'particularites', operator: 'includes', value: 'cer' }], tenseIds: coreTenses, questionCount: 10 },
  { id: 'ger-cer', label: 'Verbes en -ger et -cer', description: 'Entraînement combiné aux verbes en -ger et -cer.', group: 'spelling', criteria: [], tenseIds: coreTenses, questionCount: 10 },
  { id: 'sens-mouvement', label: 'Mouvement et déplacement', description: 'Aller, venir, partir et autres verbes de déplacement.', group: 'semantic', criteria: [{ field: 'categoriesSemantiques', operator: 'includes', value: 'mouvement' }], tenseIds: coreTenses, questionCount: 10 },
  { id: 'sens-communication', label: 'Communication', description: 'Dire, parler, répondre, expliquer et raconter.', group: 'semantic', criteria: [{ field: 'categoriesSemantiques', operator: 'includes', value: 'communication' }], tenseIds: coreTenses, questionCount: 10 },
  { id: 'sens-cognition', label: 'Pensée et connaissance', description: 'Penser, savoir, comprendre, apprendre et décider.', group: 'semantic', criteria: [{ field: 'categoriesSemantiques', operator: 'includes', value: 'cognition' }], tenseIds: coreTenses, questionCount: 10 },
  { id: 'sens-emotion', label: 'Émotions et appréciation', description: 'Aimer, préférer, craindre, rire et ressentir.', group: 'semantic', criteria: [{ field: 'categoriesSemantiques', operator: 'includes', value: 'emotion' }], tenseIds: coreTenses, questionCount: 10 },
...
export function resolveChallengePresetDefinitions(
  ...
  return definitions.map((definition) => {
    ...
    return {
      ...
      learningSupportMode: definition.group === 'cif' ? 'cif-fle' : 'normal',
      voiceMode: 'active',
      includeComplements,
      complementPlacement,
...
export function challengeConfigFromLegacyTuple(tuple: LegacyChallengeTuple): ChallengeConfig {
  return {
    ...
    questionCount: Number.isInteger(tuple[2]) && tuple[2] > 0 ? tuple[2] : 10,
    exerciseKind: 'conjugation',
    identificationSource: 'selected-verbs',
    pastSimplePronouns: 'all',
    inclusivePronouns: false,
