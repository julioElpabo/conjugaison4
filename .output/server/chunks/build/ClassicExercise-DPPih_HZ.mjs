...
import { R as validateConjugationAnswer, S as validateAnswer, ai as conjugationRequiresSubjectPronoun, aj as conjugationAnswerPlaceholder, ak as providedSubjunctiveInputPrefix, al as getAlternativeCorrections, am as impossibleSingularEndingReminderMessage, X as grammarTenseCode, an as isFutureSimpleInsteadOfNearFuture, ao as findConjugationConfusions, ap as findImpossibleSingularEnding, aq as diagnoseCoachAgreement, ar as diagnoseCoachAnswer, W as learnerErrorDetails, as as mergeLearnerErrorDetails } from '../nitro/nitro.mjs';
        ...
        if (!unref(isFinished) && unref(currentQuestion)) {
          ...
          ssrRenderList(unref(attempts), (attempt, index) => {
            ...
            ssrRenderList(attempt.question.reponsesPourCorrige.length ? attempt.question.reponsesPourCorrige : attempt.question.reponses, (expectedAnswer) => {
              _push2(`<div>${ssrInterpolate(expectedAnswer)}</div>`);
            });
...
//# sourceMappingURL=ClassicExercise-DPPih_HZ.mjs.map
