import { d as defineEventHandler, g as getRouterParam, i as getQuery, c as createError, u as useDatabase } from '../../../../../nitro/nitro.mjs';
...
const auditCases_get = defineEventHandler(async (event) => {
  ...
  if (!Number.isInteger(caractereId) || caractereId < 1) throw createError({ statusCode: 400, statusMessage: "Caract\xE8re invalide" });
  const database = useDatabase();
  const [caractereRows] = await database.execute(
    "SELECT id FROM coach_characters WHERE id=? AND status<>'disabled' LIMIT 1",
    [caractereId]
  );
  if (!caractereRows.length) throw createError({ statusCode: 404, statusMessage: "Caract\xE8re introuvable" });
  const [totalsResult, verbsResult] = await Promise.all([
    database.execute(`SELECT COUNT(*) AS totalCases,COUNT(DISTINCT vc.verbe_id) AS totalVerbs
      FROM verbesconjugues vc INNER JOIN verbes v ON v.id=vc.verbe_id
      WHERE v.est_archive=0 AND TRIM(COALESCE(vc.conjugaison1,''))<>''`),
    database.execute(`SELECT v.id,v.infinitif,v.\`participe_pr\xE9sent\` AS participePresent,
      v.\`participe_pass\xE9\` AS participePasse,v.auxiliaire,v.groupe_conjugaison AS groupeConjugaison,
  ...
  const forms = verbIds.length ? (await database.execute(`SELECT vc.verbe_id AS verbId,vc.personne_id AS personId,vc.temp_id AS tenseId,
  ...
  const verbs = verbRows.map((row) => ({
    verb: {
      ...
      meaning: row.meaning || "",
      participePresent: row.participePresent,
      participePasse: row.participePasse,
      auxiliaire: row.auxiliaire,
      groupeConjugaison: row.groupeConjugaison,
      familleConjugaison: row.familleConjugaison,
      terminaison: row.terminaison,
      typePronominal: row.typePronominal,
      estImpersonnel: Boolean(row.estImpersonnel),
      estDefectif: Boolean(row.estDefectif),
      personnesDisponibles: parsedArray(row.personnesDisponibles),
      typeHInitial: row.typeHInitial,
      niveauDifficulte: row.niveauDifficulte,
      niveauCecrl: row.niveauCecrl,
      rangFrequence: row.rangFrequence,
      registrePrincipal: row.registrePrincipal,
      formeCanonique: row.formeCanonique || row.infinitif,
      statutValidation: row.statutValidation,
      particularites: parsedArray(row.particularites),
      niveauxScolaires: parsedArray(row.niveauxScolaires),
      parcoursCif: parsedArray(row.parcoursCif),
      categoriesSemantiques: []
    },
    conjugations: forms.filter((form) => Number(form.verbId) === Number(row.id)).map((form) => ({
      personId: Number(form.personId),
      tenseId: Number(form.tenseId),
      modeId: Number(form.modeId),
      conjugaison1: form.conjugaison1,
      conjugaison2: form.conjugaison2 || "",
      conjugaison3: form.conjugaison3 || "",
      pronoun: form.pronoun || "",
      tense: form.tense,
      mode: form.mode,
      isCompound: Boolean(form.isCompound)
    }))
  }));
  const nextAfter = verbIds.at(-1) || after;
  return {
    totalCases: Number((totals == null ? void 0 : totals.totalCases) || 0),
    totalVerbs: Number((totals == null ? void 0 : totals.totalVerbs) || 0),
    verbs,
    nextAfter,
    done: verbRows.length < limit
  };
});

export { auditCases_get as default };
//# sourceMappingURL=audit-cases.get.mjs.map
