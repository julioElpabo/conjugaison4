...
interface CategoryIdRow extends RowDataPacket { id: number }

export default defineEventHandler(async (event) => {
  requireAdministrator(event)
  const body = await readBody(event)
  const orderedIds = Array.isArray(body?.orderedIds) ? body.orderedIds.map(Number) : []
  if (!orderedIds.length || orderedIds.some((id: number) => !Number.isInteger(id) || id < 1)
    || new Set(orderedIds).size !== orderedIds.length) {
    throw createError({ statusCode: 400, statusMessage: 'Ordre des catégories invalide' })
  }

  const connection = await useDatabase().getConnection()
  try {
    await connection.beginTransaction()
    const [rows] = await connection.execute<CategoryIdRow[]>(
    ...
    if (storedIds.length !== requestedIds.length || storedIds.some((id, index) => id !== requestedIds[index])) {
      throw createError({ statusCode: 409, statusMessage: 'La liste des catégories a changé, rechargez la page' })
    }
