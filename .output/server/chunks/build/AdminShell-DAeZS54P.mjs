...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "admin-shell admin-card" }, _attrs))} data-v-316a1cd6><header class="admin-shell__header" style="${ssrRenderStyle(unref(stickyHeaderStyle))}" data-v-316a1cd6><div class="admin-shell__identity" data-v-316a1cd6><span class="admin-shell__mark" aria-hidden="true" data-v-316a1cd6>A</span><div data-v-316a1cd6><strong data-v-316a1cd6>Administration</strong>`);
      if (unref(user)) {
        _push(`<small data-v-316a1cd6>${ssrInterpolate(unref(user).prenom)} ${ssrInterpolate(unref(user).nom)}</small>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div><nav class="admin-shell__nav" aria-label="Navigation d’administration" data-v-316a1cd6><!--[-->`);
      ssrRenderList(menuGroups, (group) => {
        _push(`<div class="${ssrRenderClass([[`admin-menu--${group.id}`, { active: groupIsActive(group) }], "admin-menu"])}" data-v-316a1cd6><button type="button" aria-haspopup="menu"${ssrRenderAttr("aria-expanded", unref(openMenu) === group.id)}${ssrRenderAttr("aria-controls", `admin-menu-${group.id}`)} data-v-316a1cd6>${ssrInterpolate(group.label)} <span aria-hidden="true" data-v-316a1cd6>⌄</span></button><div${ssrRenderAttr("id", `admin-menu-${group.id}`)} class="admin-menu__panel" role="menu" style="${ssrRenderStyle(unref(openMenu) === group.id ? null : { display: "none" })}" data-v-316a1cd6><!--[-->`);
      ...
      _push(`</nav><button class="admin-button admin-button--small" type="button"${ssrIncludeBooleanAttr(unref(loggingOut)) ? " disabled" : ""} data-v-316a1cd6>${ssrInterpolate(unref(loggingOut) ? "Déconnexion…" : "Se déconnecter")}</button></header>`);
      if (unref(authError)) {
        _push(`<p class="admin-shell__error admin-notice admin-notice--error" role="alert" data-v-316a1cd6>${ssrInterpolate(unref(authError))}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<main class="admin-shell__content" data-v-316a1cd6>`);
...
const __nuxt_component_1 = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main, [["__scopeId", "data-v-316a1cd6"]]), { __name: "AdminShell" });

export { __nuxt_component_0 as _, __nuxt_component_1 as a, getAdminErrorMessage as g, useAdminAuth as u };
//# sourceMappingURL=AdminShell-DAeZS54P.mjs.map
