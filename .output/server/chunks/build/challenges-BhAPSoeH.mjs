...
import { defineComponent, ref, computed, watch, withCtx, unref, createVNode, openBlock, createBlock, createCommentVNode, toDisplayString, Fragment, renderList, withDirectives, vShow, withModifiers, vModelCheckbox, vModelText, vModelSelect, isRef, createTextVNode, useTemplateRef, useSSRContext } from 'vue';
import { ssrRenderComponent, ssrRenderClass, ssrInterpolate, ssrRenderList, ssrRenderAttr, ssrRenderStyle, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderTeleport } from 'vue/server-renderer';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  __name: "DefisCleanupDialog",
  __ssrInlineRender: true,
  emits: ["close"],
  setup(__props, { emit: __emit }) {
    useAdminAuth();
    useTemplateRef("dialog");
    useTemplateRef("cancel-button");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        _push2(`<div class="cleanup-backdrop" data-v-bc5bb42e><section class="${ssrRenderClass([`cleanup-dialog--${unref(step)}`, "cleanup-dialog"])}"${ssrRenderAttr("role", unref(step) === "confirm" ? "alertdialog" : "dialog")} aria-modal="true" aria-labelledby="cleanup-title" aria-describedby="cleanup-description"${ssrRenderAttr("aria-busy", unref(loading) || unref(deleting))} tabindex="-1" data-v-bc5bb42e><div class="cleanup-icon" aria-hidden="true" data-v-bc5bb42e>${ssrInterpolate(unref(step) === "done" ? "✓" : unref(step) === "confirm" ? "!" : "▤")}</div><p class="cleanup-eyebrow" data-v-bc5bb42e>${ssrInterpolate(unref(step) === "confirm" ? "Dernière confirmation" : unref(step) === "done" ? "Nettoyage terminé" : "Entretien de la base de données")}</p><h2 id="cleanup-title" data-v-bc5bb42e>${ssrInterpolate(unref(step) === "confirm" ? "Supprimer définitivement ces défis ?" : unref(step) === "done" ? "Le nettoyage est terminé" : "Faire de la place dans la BDD")}</h2>`);
        if (unref(step) === "preview") {
          _push2(`<!--[--><p id="cleanup-description" data-v-bc5bb42e>Ce nettoyage supprime les défis enregistrés dont le lien ou le code n’a pas été chargé depuis plus de cinq ans. Les défis marqués permanents et le catalogue d’exercices sont conservés.</p><div class="cleanup-count" role="status" aria-live="polite" data-v-bc5bb42e>`);
          if (unref(loading)) {
            _push2(`<!--[-->Recherche des défis inutilisés…<!--]-->`);
          } else if (unref(preview)) {
            _push2(`<!--[--><strong data-v-bc5bb42e>${ssrInterpolate(unref(countLabel))}</strong><span data-v-bc5bb42e>${ssrInterpolate(unref(preview).count === 1 ? "défi peut être supprimé" : "défis peuvent être supprimés")}</span><small data-v-bc5bb42e>Dernière utilisation antérieure au ${ssrInterpolate(unref(cutoffLabel))}.</small><!--]-->`);
          } else {
            _push2(`<!--[-->Décompte indisponible<!--]-->`);
          }
          _push2(`</div><p class="cleanup-note" data-v-bc5bb42e>Pour les anciens défis sans historique d’utilisation, les cinq ans sont comptés à partir de la mise en place du suivi. Leur date de création ne suffit pas à les supprimer.</p>`);
          if (unref(preview)?.count === 0 && !unref(loading)) {
            _push2(`<p class="cleanup-empty" data-v-bc5bb42e>Aucun défi à supprimer pour le moment.</p>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`<!--]-->`);
        } else if (unref(step) === "confirm") {
          _push2(`<!--[--><p id="cleanup-description" data-v-bc5bb42e>Tu vas supprimer définitivement jusqu’à <strong data-v-bc5bb42e>${ssrInterpolate(unref(countLabel))} ${ssrInterpolate(unref(preview)?.count === 1 ? "défi enregistré" : "défis enregistrés")}</strong> inutilisés depuis plus de cinq ans.</p><div class="cleanup-warning" data-v-bc5bb42e>Leurs liens et leurs codes ne fonctionneront plus. Ils disparaîtront également de « Mes défis ». Cette action est irréversible.</div><p class="cleanup-note" data-v-bc5bb42e>Un défi utilisé à nouveau ou marqué permanent depuis le décompte sera conservé.</p><!--]-->`);
        } else {
          _push2(`<!--[--><p id="cleanup-description" role="status" data-v-bc5bb42e><strong data-v-bc5bb42e>${ssrInterpolate(unref(deletedCount).toLocaleString("fr-CH"))}</strong> ${ssrInterpolate(unref(deletedCount) === 1 ? "défi supprimé." : "défis supprimés.")}</p><p class="cleanup-note" data-v-bc5bb42e>Les défis utilisés au cours des cinq dernières années et les défis permanents sont conservés.</p><!--]-->`);
        }
        if (unref(error)) {
          _push2(`<p class="cleanup-error" role="alert" data-v-bc5bb42e>${ssrInterpolate(unref(error))}</p>`);
        } else {
          _push2(`<!---->`);
        }
        _push2(`<footer data-v-bc5bb42e><button type="button" class="cleanup-secondary"${ssrIncludeBooleanAttr(unref(deleting)) ? " disabled" : ""} data-v-bc5bb42e>${ssrInterpolate(unref(step) === "done" ? "Fermer" : "Annuler")}</button>`);
        if (unref(step) === "preview" && !unref(preview) && !unref(loading)) {
          _push2(`<button type="button" class="cleanup-primary" data-v-bc5bb42e>Actualiser le décompte</button>`);
        } else if (unref(step) === "preview") {
          _push2(`<button type="button" class="cleanup-primary"${ssrIncludeBooleanAttr(unref(loading) || !unref(preview)?.count) ? " disabled" : ""} data-v-bc5bb42e>Supprimer</button>`);
        } else if (unref(step) === "confirm") {
          _push2(`<button type="button" class="cleanup-danger"${ssrIncludeBooleanAttr(unref(deleting)) ? " disabled" : ""} data-v-bc5bb42e>${ssrInterpolate(unref(deleting) ? "Suppression en cours…" : "Confirmer la suppression")}</button>`);
        } else {
          _push2(`<!---->`);
        }
        _push2(`</footer></section></div>`);
      }, "body", false, _parent);
    };
  }
});
...
_sfc_main$1.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/admin/DefisCleanupDialog.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const __nuxt_component_2 = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main$1, [["__scopeId", "data-v-bc5bb42e"]]), { __name: "AdminDefisCleanupDialog" });
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const PUBLICATION_LOCALES = [
      ...
      { value: "nl", label: "🇧🇪 Nederlands (België)" }
    ];
    ...
    const publicationDraft = ref(null);
    const publicationLocale = ref("fr");
    ...
    const publicationUrl = computed(() => publicationDraft.value?.slug ? `/${publicationDraft.value.locale}/defis/${publicationDraft.value.slug}` : "");
    const publishedSlugChanged = computed(() => Boolean(
      originalPublishedSlug && publicationDraft.value?.slug && originalPublishedSlug !== publicationDraft.value.slug
    ));
    ...
    function emptyPublication(presetId, locale) {
      return {
        id: 0,
        presetId,
        locale,
    ...
    function setPublicationDraft(locale) {
      suspendAutosave = true;
      publicationLocale.value = locale;
      publicationDraft.value = clone(
        publications.value.find((publication) => publication.locale === locale) ?? emptyPublication(presetDraft.value?.databaseId ?? 0, locale)
      );
    ...
    async function loadPublications(presetId) {
      ...
      try {
        const response = await $fetch(`/api/admin/challenge-presets/${presetId}/publications`);
    ...
    async function selectPublicationLocale(locale) {
      await flushPublicationAutosave();
      setPublicationDraft(locale);
    }
    function publicationState(locale) {
    ...
    function publicationCanBeSaved(draft) {
      if (!draft.presetId) return false;
      if (!draft.isPublished) return true;
      return Boolean(draft.slug.trim() && draft.title.trim() && draft.metaTitle.trim() && draft.description.trim() && draft.metaDescription.trim());
    }
    ...
    function schedulePublicationAutosave() {
      ...
      publicationAutosaveTimer = setTimeout(() => {
    ...
    async function savePublication() {
      ...
      try {
        const response = await $fetch(`/api/admin/challenge-presets/${draft.presetId}/publications/${draft.locale}`, {
          method: "PUT",
          body: {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_AdminDefisCleanupDialog = __nuxt_component_2;
          ...
          if (_push2) {
            _push2(ssrRenderComponent(_component_AdminShell, null, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`<div class="challenge-admin" data-v-08a3fdff${_scopeId2}><header class="admin-section-heading challenge-admin__heading" data-v-08a3fdff${_scopeId2}><div data-v-08a3fdff${_scopeId2}><p class="admin-eyebrow" data-v-08a3fdff${_scopeId2}>Catalogue d’exercices</p><h1 data-v-08a3fdff${_scopeId2}>Gestion des défis</h1><p class="admin-muted" data-v-08a3fdff${_scopeId2}>Configure les défis proposés, leurs verbes, leurs temps et toutes leurs options.</p></div><div class="challenge-admin__tools" data-v-08a3fdff${_scopeId2}><button type="button" class="challenge-cleanup-button" data-v-08a3fdff${_scopeId2}>Faire de la place dans la BDD</button><div class="challenge-tabs" role="tablist" aria-label="Gestion des défis" data-v-08a3fdff${_scopeId2}><button type="button" class="${ssrRenderClass({ active: unref(tab) === "presets" })}" data-v-08a3fdff${_scopeId2}>Défis</button><button type="button" class="${ssrRenderClass({ active: unref(tab) === "categories" })}" data-v-08a3fdff${_scopeId2}>Catégories</button></div></div></header>`);
                  if (unref(cleanupOpen)) {
                    _push3(ssrRenderComponent(_component_AdminDefisCleanupDialog, {
                      onClose: ($event) => cleanupOpen.value = false
                    }, null, _parent3, _scopeId2));
                  } else {
                    _push3(`<!---->`);
                  }
                  if (unref(error)) {
                    _push3(`<p class="admin-notice admin-notice--error" role="alert" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(error))}</p>`);
                  } else {
                    _push3(`<!---->`);
                  }
                  if (unref(success)) {
                    _push3(`<p class="admin-notice admin-notice--success" role="status" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(success))}</p>`);
                  } else {
                    _push3(`<!---->`);
                  }
                  if (unref(loading)) {
                    _push3(`<p class="admin-muted" data-v-08a3fdff${_scopeId2}>Chargement…</p>`);
                  } else if (unref(tab) === "presets") {
                    _push3(`<div class="challenge-workspace" data-v-08a3fdff${_scopeId2}><aside class="challenge-list admin-card" data-v-08a3fdff${_scopeId2}><header data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>Défis</strong><button type="button" class="admin-button admin-button--small admin-button--primary" data-v-08a3fdff${_scopeId2}>Nouveau</button></header><!--[-->`);
                    ssrRenderList(unref(presetGroups), (group) => {
                      _push3(`<section class="challenge-list__group" data-v-08a3fdff${_scopeId2}><h2 data-v-08a3fdff${_scopeId2}><button type="button"${ssrRenderAttr("aria-expanded", unref(openPresetCategoryId) === group.category.id)}${ssrRenderAttr("aria-controls", `preset-category-${group.category.id}`)} data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>${ssrInterpolate(group.category.name)}</span><small data-v-08a3fdff${_scopeId2}>${ssrInterpolate(group.presets.length)}</small><i aria-hidden="true" data-v-08a3fdff${_scopeId2}>⌄</i></button></h2><div${ssrRenderAttr("id", `preset-category-${group.category.id}`)} class="challenge-list__items" style="${ssrRenderStyle(unref(openPresetCategoryId) === group.category.id ? null : { display: "none" })}" data-v-08a3fdff${_scopeId2}><!--[-->`);
                      ssrRenderList(group.presets, (preset) => {
                        _push3(`<button type="button" draggable="true" class="${ssrRenderClass({ selected: preset.databaseId === unref(presetDraft)?.databaseId, dragging: preset.databaseId === unref(draggedPresetId), "drag-over": preset.databaseId === unref(dragOverPresetId) && preset.databaseId !== unref(draggedPresetId) })}"${ssrRenderAttr("title", `Glisser pour déplacer ${preset.label}`)} data-v-08a3fdff${_scopeId2}><b class="drag-handle" aria-hidden="true" data-v-08a3fdff${_scopeId2}>⠿</b><span data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>${ssrInterpolate(preset.label)}</strong><small data-v-08a3fdff${_scopeId2}>№ ${ssrInterpolate(preset.sortOrder)} · ${ssrInterpolate(preset.verbIds.length)} verbes · ${ssrInterpolate(preset.tenseIds.length)} temps</small></span><i class="${ssrRenderClass({ inactive: !preset.isActive })}" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(preset.isActive ? "Actif" : "Masqué")}</i></button>`);
                      });
                      _push3(`<!--]--></div></section>`);
                    });
                    _push3(`<!--]--></aside>`);
                    if (unref(presetDraft)) {
                      _push3(`<form class="challenge-editor admin-card" data-v-08a3fdff${_scopeId2}><div class="editor-title" data-v-08a3fdff${_scopeId2}><div data-v-08a3fdff${_scopeId2}><p class="admin-eyebrow" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(presetDraft).databaseId ? "Défi existant" : "Nouveau défi")}</p><h2 data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(presetDraft).label || "Sans nom")}</h2></div><label class="switch" data-v-08a3fdff${_scopeId2}><input${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).isActive) ? ssrLooseContain(unref(presetDraft).isActive, null) : unref(presetDraft).isActive) ? " checked" : ""} type="checkbox" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Visible</span></label></div><section class="editor-section" data-v-08a3fdff${_scopeId2}><h3 data-v-08a3fdff${_scopeId2}>Informations</h3><div class="field-grid" data-v-08a3fdff${_scopeId2}><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Nom *</span><input${ssrRenderAttr("value", unref(presetDraft).label)} required data-v-08a3fdff${_scopeId2}></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Identifiant *</span><input${ssrRenderAttr("value", unref(presetDraft).id)} required pattern="[A-Za-z0-9][A-Za-z0-9_-]*" data-v-08a3fdff${_scopeId2}></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Catégorie *</span><select required data-v-08a3fdff${_scopeId2}><!--[-->`);
                      ssrRenderList(unref(sortedCategories), (category) => {
                        _push3(`<option${ssrRenderAttr("value", category.id)} data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).categoryId) ? ssrLooseContain(unref(presetDraft).categoryId, category.id) : ssrLooseEqual(unref(presetDraft).categoryId, category.id)) ? " selected" : ""}${_scopeId2}>${ssrInterpolate(category.name)}</option>`);
                      });
                      _push3(`<!--]--></select></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Ordre dans la catégorie</span><input${ssrRenderAttr("value", unref(presetDraft).sortOrder)} type="number" min="1"${ssrRenderAttr("max", unref(presets).filter((preset) => preset.categoryId === unref(presetDraft).categoryId && preset.databaseId !== unref(presetDraft).databaseId).length + 1)} required data-v-08a3fdff${_scopeId2}></label><label class="admin-field wide" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Description</span><textarea rows="2" maxlength="500" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(presetDraft).description)}</textarea></label></div></section><section class="editor-section" data-v-08a3fdff${_scopeId2}><div class="section-heading" data-v-08a3fdff${_scopeId2}><div data-v-08a3fdff${_scopeId2}><h3 data-v-08a3fdff${_scopeId2}>Verbes</h3><small data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(presetDraft).verbIds.length)} sélectionné(s)</small></div></div><div class="${ssrRenderClass([{ empty: !unref(selectedVerbs).length }, "selected-verbs"])}" data-v-08a3fdff${_scopeId2}><div class="selected-verbs__heading" data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>Verbes sélectionnés</strong>`);
                      if (unref(selectedVerbs).length) {
                        _push3(`<button type="button" data-v-08a3fdff${_scopeId2}>Tout retirer</button>`);
                      } else {
                        _push3(`<!---->`);
                      }
                      _push3(`</div>`);
                      if (!unref(selectedVerbs).length) {
                        _push3(`<p data-v-08a3fdff${_scopeId2}>Aucun verbe sélectionné.</p>`);
                      } else {
                        _push3(`<div class="selected-verbs__badges" data-v-08a3fdff${_scopeId2}><!--[-->`);
                        ssrRenderList(unref(selectedVerbs), (verb) => {
                          _push3(`<button type="button"${ssrRenderAttr("aria-label", `Retirer ${verb.infinitif}`)}${ssrRenderAttr("title", `Retirer ${verb.infinitif}`)} data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>${ssrInterpolate(verb.infinitif)}</span><i aria-hidden="true" data-v-08a3fdff${_scopeId2}>×</i></button>`);
                        });
                        _push3(`<!--]--></div>`);
                      }
                      _push3(`</div><div class="verb-catalogue-heading" data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>Tous les verbes</strong><input${ssrRenderAttr("value", unref(verbSearch))} class="search" type="search" placeholder="Rechercher un verbe…" data-v-08a3fdff${_scopeId2}></div><div class="selection-actions" data-v-08a3fdff${_scopeId2}><button type="button" class="admin-button admin-button--small" data-v-08a3fdff${_scopeId2}>Sélectionner les résultats</button><button type="button" class="admin-button admin-button--small" data-v-08a3fdff${_scopeId2}>Retirer les résultats</button></div><div class="choice-grid verb-grid" data-v-08a3fdff${_scopeId2}><!--[-->`);
                      ssrRenderList(unref(filteredVerbs), (verb) => {
                        _push3(`<label class="${ssrRenderClass({ selected: unref(presetDraft).verbIds.includes(verb.id) })}" data-v-08a3fdff${_scopeId2}><input type="checkbox"${ssrIncludeBooleanAttr(unref(presetDraft).verbIds.includes(verb.id)) ? " checked" : ""} data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>${ssrInterpolate(verb.infinitif)}</span></label>`);
                      });
                      _push3(`<!--]--></div></section><section class="editor-section" data-v-08a3fdff${_scopeId2}><div class="section-heading" data-v-08a3fdff${_scopeId2}><div data-v-08a3fdff${_scopeId2}><h3 data-v-08a3fdff${_scopeId2}>Temps</h3><small data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(presetDraft).tenseIds.length)} sélectionné(s)</small></div></div><div class="tense-groups" data-v-08a3fdff${_scopeId2}><!--[-->`);
                      ssrRenderList(unref(tensesByMode), (group) => {
                        _push3(`<div data-v-08a3fdff${_scopeId2}><header data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>${ssrInterpolate(group.mode.name)}</strong><span data-v-08a3fdff${_scopeId2}><button type="button" data-v-08a3fdff${_scopeId2}>Tous</button><button type="button" data-v-08a3fdff${_scopeId2}>Aucun</button></span></header><div class="choice-grid" data-v-08a3fdff${_scopeId2}><!--[-->`);
                        ssrRenderList(group.tenses, (tense) => {
                          _push3(`<label class="${ssrRenderClass({ selected: unref(presetDraft).tenseIds.includes(tense.id) })}" data-v-08a3fdff${_scopeId2}><input type="checkbox"${ssrIncludeBooleanAttr(unref(presetDraft).tenseIds.includes(tense.id)) ? " checked" : ""} data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>${ssrInterpolate(tense.name)}</span></label>`);
                        });
                        _push3(`<!--]--></div></div>`);
                      });
                      _push3(`<!--]--></div></section><section class="editor-section" data-v-08a3fdff${_scopeId2}><h3 data-v-08a3fdff${_scopeId2}>Options</h3><div class="field-grid" data-v-08a3fdff${_scopeId2}><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Nombre de questions</span><input${ssrRenderAttr("value", unref(presetDraft).questionCount)} type="number" min="1" max="100" required data-v-08a3fdff${_scopeId2}></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Type d’exercice</span><select data-v-08a3fdff${_scopeId2}><option value="conjugation" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).exerciseKind) ? ssrLooseContain(unref(presetDraft).exerciseKind, "conjugation") : ssrLooseEqual(unref(presetDraft).exerciseKind, "conjugation")) ? " selected" : ""}${_scopeId2}>Conjugaison</option><option value="tense-identification" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).exerciseKind) ? ssrLooseContain(unref(presetDraft).exerciseKind, "tense-identification") : ssrLooseEqual(unref(presetDraft).exerciseKind, "tense-identification")) ? " selected" : ""}${_scopeId2}>Identifier le temps</option><option value="mixed" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).exerciseKind) ? ssrLooseContain(unref(presetDraft).exerciseKind, "mixed") : ssrLooseEqual(unref(presetDraft).exerciseKind, "mixed")) ? " selected" : ""}${_scopeId2}>Un mélange des deux</option></select></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Mode CIF / FLE</span><select data-v-08a3fdff${_scopeId2}><option value="normal" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).learningSupportMode) ? ssrLooseContain(unref(presetDraft).learningSupportMode, "normal") : ssrLooseEqual(unref(presetDraft).learningSupportMode, "normal")) ? " selected" : ""}${_scopeId2}>Non</option><option value="cif-fle" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).learningSupportMode) ? ssrLooseContain(unref(presetDraft).learningSupportMode, "cif-fle") : ssrLooseEqual(unref(presetDraft).learningSupportMode, "cif-fle")) ? " selected" : ""}${_scopeId2}>Oui — aide audio pour personnes allophones</option></select></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Pronoms au passé simple</span><select data-v-08a3fdff${_scopeId2}><option value="all" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).pastSimplePronouns) ? ssrLooseContain(unref(presetDraft).pastSimplePronouns, "all") : ssrLooseEqual(unref(presetDraft).pastSimplePronouns, "all")) ? " selected" : ""}${_scopeId2}>Tous les pronoms</option><option value="third-person-only" data-v-08a3fdff${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).pastSimplePronouns) ? ssrLooseContain(unref(presetDraft).pastSimplePronouns, "third-person-only") : ssrLooseEqual(unref(presetDraft).pastSimplePronouns, "third-person-only")) ? " selected" : ""}${_scopeId2}>3e personnes seulement</option></select></label><label class="check-line" data-v-08a3fdff${_scopeId2}><input${ssrIncludeBooleanAttr(Array.isArray(unref(presetDraft).inclusivePronouns) ? ssrLooseContain(unref(presetDraft).inclusivePronouns, null) : unref(presetDraft).inclusivePronouns) ? " checked" : ""} type="checkbox" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Inclure les pronoms « iel / iels »</span></label></div><div data-v-08a3fdff${_scopeId2}><strong class="option-title" data-v-08a3fdff${_scopeId2}>Compléments proposés</strong><div class="choice-grid complement-grid" data-v-08a3fdff${_scopeId2}><!--[-->`);
                      ssrRenderList(COMPLEMENT_OPTIONS, (option) => {
                        _push3(`<label class="${ssrRenderClass({ selected: unref(presetDraft).complementOptions.includes(option.value) })}" data-v-08a3fdff${_scopeId2}><input type="checkbox"${ssrIncludeBooleanAttr(unref(presetDraft).complementOptions.includes(option.value)) ? " checked" : ""} data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>${ssrInterpolate(option.label)}</span></label>`);
                      });
                      _push3(`<!--]--></div></div></section><section id="seo-publications" class="editor-section publication-editor" data-v-08a3fdff${_scopeId2}><div class="section-heading" data-v-08a3fdff${_scopeId2}><div data-v-08a3fdff${_scopeId2}><h3 data-v-08a3fdff${_scopeId2}>Publication SEO</h3><small data-v-08a3fdff${_scopeId2}>Pages publiques multilingues rattachées à ce défi officiel</small></div>`);
                      if (unref(presetDraft).databaseId) {
                        _push3(`<button type="button" class="admin-button admin-button--small"${ssrIncludeBooleanAttr(unref(publicationReloading)) ? " disabled" : ""} data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(publicationReloading) ? "Rechargement…" : "Recharger les textes")}</button>`);
                      } else {
                        _push3(`<!---->`);
                      }
                      _push3(`</div>`);
                      if (!unref(presetDraft).databaseId) {
                        _push3(`<p class="admin-notice" data-v-08a3fdff${_scopeId2}>Enregistre d’abord le défi pour pouvoir préparer ses pages publiques.</p>`);
                      } else if (unref(publicationDraft)) {
                        _push3(`<!--[--><div class="publication-tabs" role="tablist" aria-label="Langue de la publication" data-v-08a3fdff${_scopeId2}><!--[-->`);
                        ssrRenderList(PUBLICATION_LOCALES, (localeOption) => {
                          _push3(`<button type="button" role="tab"${ssrRenderAttr("aria-selected", unref(publicationLocale) === localeOption.value)} class="${ssrRenderClass({ active: unref(publicationLocale) === localeOption.value, ready: unref(publications).some((publication) => publication.locale === localeOption.value && publication.isPublished) })}" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>${ssrInterpolate(localeOption.label)}</span><small data-v-08a3fdff${_scopeId2}>${ssrInterpolate(publicationState(localeOption.value))}</small></button>`);
                        });
                        _push3(`<!--]--></div><div class="publication-switches" data-v-08a3fdff${_scopeId2}><label class="switch" data-v-08a3fdff${_scopeId2}><input${ssrIncludeBooleanAttr(Array.isArray(unref(publicationDraft).isPublished) ? ssrLooseContain(unref(publicationDraft).isPublished, null) : unref(publicationDraft).isPublished) ? " checked" : ""} type="checkbox" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Publiée</span></label></div><p class="admin-muted" data-v-08a3fdff${_scopeId2}>Une publication peut rester en brouillon. Lorsqu’elle est publiée, elle apparaît sur le site, devient indexable et est ajoutée au sitemap.</p><div class="field-grid" data-v-08a3fdff${_scopeId2}><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Slug *</span><input${ssrRenderAttr("value", unref(publicationDraft).slug)} placeholder="defi-verbes-en-ger" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" data-v-08a3fdff${_scopeId2}></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Titre visible (H1) *</span><input${ssrRenderAttr("value", unref(publicationDraft).title)} maxlength="160" data-v-08a3fdff${_scopeId2}></label><label class="admin-field wide" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Meta title *</span><input${ssrRenderAttr("value", unref(publicationDraft).metaTitle)} maxlength="160" data-v-08a3fdff${_scopeId2}></label><label class="admin-field wide" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Description visible *</span><textarea rows="3" maxlength="1000" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(publicationDraft).description)}</textarea></label><label class="admin-field wide" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Meta description *</span><textarea rows="3" maxlength="320" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(publicationDraft).metaDescription)}</textarea></label></div>`);
                        if (unref(publicationUrl)) {
                          _push3(`<p class="publication-url" data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>URL :</strong> ${ssrInterpolate(unref(publicationUrl))}</p>`);
                        } else {
                          _push3(`<!---->`);
                        }
                        if (unref(publishedSlugChanged)) {
                          _push3(`<p class="admin-notice admin-notice--warning" data-v-08a3fdff${_scopeId2}>Le slug publié a changé. L’ancienne URL sera conservée comme redirection permanente lors de l’enregistrement.</p>`);
                        } else {
                          _push3(`<!---->`);
                        }
                        _push3(`<p class="${ssrRenderClass([`is-${unref(publicationAutosaveState)}`, "autosave-status publication-status"])}" aria-live="polite" data-v-08a3fdff${_scopeId2}><i aria-hidden="true" data-v-08a3fdff${_scopeId2}></i>${ssrInterpolate(unref(publicationAutosaveLabel))} `);
                        if (unref(publicationAutosaveState) === "error") {
                          _push3(`<button type="button" data-v-08a3fdff${_scopeId2}>Réessayer</button>`);
                        } else {
                          _push3(`<!---->`);
                        }
                        _push3(`</p><!--]-->`);
                      ...
                      _push3(`</section><footer class="editor-actions" data-v-08a3fdff${_scopeId2}>`);
                      if (unref(presetDraft).databaseId) {
                        _push3(`<button type="button" class="admin-button danger"${ssrIncludeBooleanAttr(unref(saving)) ? " disabled" : ""} data-v-08a3fdff${_scopeId2}>Supprimer</button>`);
                      } else {
                        _push3(`<!---->`);
                      }
                      _push3(`<span data-v-08a3fdff${_scopeId2}></span><p class="${ssrRenderClass([`is-${unref(presetAutosaveState)}`, "autosave-status"])}" aria-live="polite" data-v-08a3fdff${_scopeId2}><i aria-hidden="true" data-v-08a3fdff${_scopeId2}></i>${ssrInterpolate(unref(presetAutosaveLabel))} `);
                      ...
                      _push3(`<div class="empty admin-card" data-v-08a3fdff${_scopeId2}><p data-v-08a3fdff${_scopeId2}>Sélectionne un défi ou crée-en un nouveau.</p></div>`);
                    }
                    _push3(`</div>`);
                  } else {
                    _push3(`<div class="challenge-workspace category-workspace" data-v-08a3fdff${_scopeId2}><aside class="challenge-list category-list admin-card" data-v-08a3fdff${_scopeId2}><header data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>Catégories</strong><button type="button" class="admin-button admin-button--small admin-button--primary" data-v-08a3fdff${_scopeId2}>Nouvelle</button></header><!--[-->`);
                    ssrRenderList(unref(sortedCategories), (category) => {
                      _push3(`<button type="button" draggable="true" class="${ssrRenderClass({ selected: category.id === unref(categoryDraft)?.id, dragging: category.id === unref(draggedCategoryId), "drag-over": category.id === unref(dragOverCategoryId) && category.id !== unref(draggedCategoryId) })}"${ssrRenderAttr("title", `Glisser pour déplacer ${category.name}`)} data-v-08a3fdff${_scopeId2}><b class="drag-handle" aria-hidden="true" data-v-08a3fdff${_scopeId2}>⠿</b><span data-v-08a3fdff${_scopeId2}><strong data-v-08a3fdff${_scopeId2}>${ssrInterpolate(category.name)}</strong><small data-v-08a3fdff${_scopeId2}>№ ${ssrInterpolate(category.sortOrder)} · ${ssrInterpolate(unref(presets).filter((preset) => preset.categoryId === category.id).length)} défi(s)</small></span><i class="${ssrRenderClass({ inactive: !category.isActive })}" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(category.isActive ? "Active" : "Masquée")}</i></button>`);
                    });
                    _push3(`<!--]--></aside>`);
                    if (unref(categoryDraft)) {
                      _push3(`<form class="challenge-editor admin-card category-editor" data-v-08a3fdff${_scopeId2}><div class="editor-title" data-v-08a3fdff${_scopeId2}><div data-v-08a3fdff${_scopeId2}><p class="admin-eyebrow" data-v-08a3fdff${_scopeId2}>Catégorie</p><h2 data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(categoryDraft).name || "Sans nom")}</h2></div><label class="switch" data-v-08a3fdff${_scopeId2}><input${ssrIncludeBooleanAttr(Array.isArray(unref(categoryDraft).isActive) ? ssrLooseContain(unref(categoryDraft).isActive, null) : unref(categoryDraft).isActive) ? " checked" : ""} type="checkbox" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Visible</span></label></div><div class="field-grid" data-v-08a3fdff${_scopeId2}><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Nom *</span><input${ssrRenderAttr("value", unref(categoryDraft).name)} required data-v-08a3fdff${_scopeId2}></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Identifiant *</span><input${ssrRenderAttr("value", unref(categoryDraft).slug)} required data-v-08a3fdff${_scopeId2}></label><label class="admin-field" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Ordre</span><input${ssrRenderAttr("value", unref(categoryDraft).sortOrder)} type="number" required data-v-08a3fdff${_scopeId2}></label><label class="admin-field wide" data-v-08a3fdff${_scopeId2}><span data-v-08a3fdff${_scopeId2}>Description</span><textarea rows="4" maxlength="500" data-v-08a3fdff${_scopeId2}>${ssrInterpolate(unref(categoryDraft).description)}</textarea></label></div><p class="admin-muted" data-v-08a3fdff${_scopeId2}>Une catégorie masquée masque aussi tous ses défis dans le catalogue public.</p><footer class="editor-actions" data-v-08a3fdff${_scopeId2}>`);
                      if (unref(categoryDraft).id) {
                        _push3(`<button type="button" class="admin-button danger"${ssrIncludeBooleanAttr(unref(saving)) ? " disabled" : ""} data-v-08a3fdff${_scopeId2}>Supprimer</button>`);
                      } else {
                        _push3(`<!---->`);
                      }
                      _push3(`<span data-v-08a3fdff${_scopeId2}></span><p class="${ssrRenderClass([`is-${unref(categoryAutosaveState)}`, "autosave-status"])}" aria-live="polite" data-v-08a3fdff${_scopeId2}><i aria-hidden="true" data-v-08a3fdff${_scopeId2}></i>${ssrInterpolate(unref(categoryAutosaveLabel))} `);
                  ...
                  return [
                    createVNode("div", { class: "challenge-admin" }, [
                      createVNode("header", { class: "admin-section-heading challenge-admin__heading" }, [
                        ...
                        createVNode("div", { class: "challenge-admin__tools" }, [
                          createVNode("button", {
                            type: "button",
                            class: "challenge-cleanup-button",
                            onClick: ($event) => cleanupOpen.value = true
                          }, "Faire de la place dans la BDD", 8, ["onClick"]),
                          createVNode("div", {
                      ...
                      unref(cleanupOpen) ? (openBlock(), createBlock(_component_AdminDefisCleanupDialog, {
                        key: 0,
                        onClose: ($event) => cleanupOpen.value = false
                      ...
                      unref(loading) ? (openBlock(), createBlock("p", {
                        ...
                        unref(presetDraft) ? (openBlock(), createBlock("form", {
                            ...
                            createVNode("div", { class: "field-grid" }, [
                              ...
                              createVNode("label", { class: "admin-field" }, [
                                ...
                                withDirectives(createVNode("select", {
                                  ...
                                  createVNode("option", { value: "mixed" }, "Un mélange des deux")
                                ], 8, ["onUpdate:modelValue"]), [
                                  [vModelSelect, unref(presetDraft).exerciseKind]
                                ])
                              ]),
                              createVNode("label", { class: "admin-field" }, [
                                createVNode("span", null, "Mode CIF / FLE"),
                                withDirectives(createVNode("select", {
                                  "onUpdate:modelValue": ($event) => unref(presetDraft).learningSupportMode = $event
                                }, [
                                  createVNode("option", { value: "normal" }, "Non"),
                                  createVNode("option", { value: "cif-fle" }, "Oui — aide audio pour personnes allophones")
                                ], 8, ["onUpdate:modelValue"]), [
                                  [vModelSelect, unref(presetDraft).learningSupportMode]
                                ])
                              ]),
                              createVNode("label", { class: "admin-field" }, [
                          ...
                          createVNode("section", {
                            id: "seo-publications",
                            class: "editor-section publication-editor"
                          }, [
                            createVNode("div", { class: "section-heading" }, [
                              createVNode("div", null, [
                                createVNode("h3", null, "Publication SEO"),
                                createVNode("small", null, "Pages publiques multilingues rattachées à ce défi officiel")
                              ]),
                              unref(presetDraft).databaseId ? (openBlock(), createBlock("button", {
                                key: 0,
                                type: "button",
                                class: "admin-button admin-button--small",
                                disabled: unref(publicationReloading),
                                onClick: reloadPublicationsFromDatabase
                              }, toDisplayString(unref(publicationReloading) ? "Rechargement…" : "Recharger les textes"), 9, ["disabled"])) : createCommentVNode("", true)
                            ]),
                            !unref(presetDraft).databaseId ? (openBlock(), createBlock("p", {
                              key: 0,
                              class: "admin-notice"
                            }, "Enregistre d’abord le défi pour pouvoir préparer ses pages publiques.")) : unref(publicationDraft) ? (openBlock(), createBlock(Fragment, { key: 1 }, [
                              createVNode("div", {
                                class: "publication-tabs",
                                role: "tablist",
                                "aria-label": "Langue de la publication"
                              }, [
                                (openBlock(), createBlock(Fragment, null, renderList(PUBLICATION_LOCALES, (localeOption) => {
                                  return createVNode("button", {
                                    key: localeOption.value,
                                    type: "button",
                                    role: "tab",
                                    "aria-selected": unref(publicationLocale) === localeOption.value,
                                    class: { active: unref(publicationLocale) === localeOption.value, ready: unref(publications).some((publication) => publication.locale === localeOption.value && publication.isPublished) },
                                    onClick: ($event) => selectPublicationLocale(localeOption.value)
                                  }, [
                                    createVNode("span", null, toDisplayString(localeOption.label), 1),
                                    createVNode("small", null, toDisplayString(publicationState(localeOption.value)), 1)
                                  ], 10, ["aria-selected", "onClick"]);
                                }), 64))
                              ]),
                              createVNode("div", { class: "publication-switches" }, [
                                createVNode("label", { class: "switch" }, [
                                  withDirectives(createVNode("input", {
                                    "onUpdate:modelValue": ($event) => unref(publicationDraft).isPublished = $event,
                                    type: "checkbox",
                                    onChange: updatePublicationVisibility
                                  }, null, 40, ["onUpdate:modelValue"]), [
                                    [vModelCheckbox, unref(publicationDraft).isPublished]
                                  ]),
                                  createVNode("span", null, "Publiée")
                                ])
                              ]),
                              createVNode("p", { class: "admin-muted" }, "Une publication peut rester en brouillon. Lorsqu’elle est publiée, elle apparaît sur le site, devient indexable et est ajoutée au sitemap."),
                              createVNode("div", { class: "field-grid" }, [
                                createVNode("label", { class: "admin-field" }, [
                                  createVNode("span", null, "Slug *"),
                                  withDirectives(createVNode("input", {
                                    "onUpdate:modelValue": ($event) => unref(publicationDraft).slug = $event,
                                    placeholder: "defi-verbes-en-ger",
                                    pattern: "[a-z0-9]+(?:-[a-z0-9]+)*"
                                  }, null, 8, ["onUpdate:modelValue"]), [
                                    [vModelText, unref(publicationDraft).slug]
                                  ])
                                ]),
                                createVNode("label", { class: "admin-field" }, [
                                  createVNode("span", null, "Titre visible (H1) *"),
                                  withDirectives(createVNode("input", {
                                    "onUpdate:modelValue": ($event) => unref(publicationDraft).title = $event,
                                    maxlength: "160"
                                  }, null, 8, ["onUpdate:modelValue"]), [
                                    [vModelText, unref(publicationDraft).title]
                                  ])
                                ]),
                                createVNode("label", { class: "admin-field wide" }, [
                                  createVNode("span", null, "Meta title *"),
                                  withDirectives(createVNode("input", {
                                    "onUpdate:modelValue": ($event) => unref(publicationDraft).metaTitle = $event,
                                    maxlength: "160"
                                  }, null, 8, ["onUpdate:modelValue"]), [
                                    [vModelText, unref(publicationDraft).metaTitle]
                                  ])
                                ]),
                                createVNode("label", { class: "admin-field wide" }, [
                                  createVNode("span", null, "Description visible *"),
                                  withDirectives(createVNode("textarea", {
                                    "onUpdate:modelValue": ($event) => unref(publicationDraft).description = $event,
                                    rows: "3",
                                    maxlength: "1000"
                                  }, null, 8, ["onUpdate:modelValue"]), [
                                    [vModelText, unref(publicationDraft).description]
                                  ])
                                ]),
                                createVNode("label", { class: "admin-field wide" }, [
                                  createVNode("span", null, "Meta description *"),
                                  withDirectives(createVNode("textarea", {
                                    "onUpdate:modelValue": ($event) => unref(publicationDraft).metaDescription = $event,
                                    rows: "3",
                                    maxlength: "320"
                                  }, null, 8, ["onUpdate:modelValue"]), [
                                    [vModelText, unref(publicationDraft).metaDescription]
                                  ])
                                ])
                              ]),
                              unref(publicationUrl) ? (openBlock(), createBlock("p", {
                                key: 0,
                                class: "publication-url"
                              }, [
                                createVNode("strong", null, "URL :"),
                                createTextVNode(" " + toDisplayString(unref(publicationUrl)), 1)
                              ])) : createCommentVNode("", true),
                              unref(publishedSlugChanged) ? (openBlock(), createBlock("p", {
                                key: 1,
                                class: "admin-notice admin-notice--warning"
                              }, "Le slug publié a changé. L’ancienne URL sera conservée comme redirection permanente lors de l’enregistrement.")) : createCommentVNode("", true),
                              createVNode("p", {
                                class: ["autosave-status publication-status", `is-${unref(publicationAutosaveState)}`],
                                "aria-live": "polite"
                              }, [
                                createVNode("i", { "aria-hidden": "true" }),
                                createTextVNode(toDisplayString(unref(publicationAutosaveLabel)) + " ", 1),
                                unref(publicationAutosaveState) === "error" ? (openBlock(), createBlock("button", {
                                  key: 0,
                                  type: "button",
...
const challenges = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-08a3fdff"]]);

export { challenges as default };
//# sourceMappingURL=challenges-BhAPSoeH.mjs.map
