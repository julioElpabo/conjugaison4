...
interface SentenceTargetRow extends RowDataPacket {
  id: number
  targetText: string
  targetStart: number
}
...
function closestOccurrence(sentence: string, form: string, reference: number) {
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ status?: unknown, note?: unknown, targetText?: unknown, sentenceText?: unknown }>(event)
  ...
  if (requestedTargetText !== null && (!requestedTargetText || requestedTargetText.length > 255)) {
    throw createError({ statusCode: 400, statusMessage: 'La forme ciblée doit contenir entre 1 et 255 caractères' })
  }
  const requestedSentenceText = typeof body.sentenceText === 'string'
    ? body.sentenceText.replace(/\s+/gu, ' ').trim()
    : null
  if (requestedSentenceText !== null && (!requestedSentenceText || requestedSentenceText.length > 1000)) {
    throw createError({ statusCode: 400, statusMessage: 'La phrase doit contenir entre 1 et 1 000 caractères' })
  }
  const database = useDatabase()
  const connection = await database.getConnection()
  try {
    await connection.beginTransaction()
    const [[target]] = await connection.execute<TargetRow[]>(`
    ...
    let effectiveStatus = requestedStatus as ReviewStatus
    let message = ''
    if (effectiveStatus === 'validated') {
      const [[selection]] = await connection.execute<CountRow[]>(`
      ...
      if (Number(selection?.count || 0) >= 10) {
        effectiveStatus = 'reserve'
        message = 'La limite de 10 citations validées pour ce verbe, ce temps et cette personne est atteinte : phrase mise en réserve.'
      }
    }

    const workingSentence = requestedSentenceText ?? target.sentenceText
    const workingTargetText = requestedTargetText ?? target.targetText
    ...
    if (requestedSentenceText !== null) {
      const [sentenceTargets] = await connection.execute<SentenceTargetRow[]>(`
      ...
      const adjustedTargets = sentenceTargets.map(sentenceTarget => {
        ...
        if (start < 0) {
          throw createError({ statusCode: 400, statusMessage: `La forme « ${form} » doit rester présente dans la phrase` })
        }
        return { id: Number(sentenceTarget.id), start, end: start + form.length }
      })
      const words = requestedSentenceText.match(/[\p{L}\p{N}]+(?:[’'-][\p{L}\p{N}]+)*/gu)?.length || 0
      ...
      for (const adjusted of adjustedTargets) {
        await connection.execute('UPDATE literary_targets SET target_start=?,target_end=? WHERE id=?', [
          adjusted.start, adjusted.end, adjusted.id,
        ])
      }
    }
