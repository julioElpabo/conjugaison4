...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  props: {
    ...
    exerciseKind: {},
    challenge: {},
    existingChallengeCode: {},
    options: {},
    requestedQuestionCount: {},
    regenerating: { type: Boolean },
    analyticsMetadata: {}
  },
  emits: ["close", "updateOptions", "challengeCodeCreated", "regenerate"],
  setup(__props, { emit: __emit }) {
    ...
    const isIdentificationQuestion = (question) => (question.exerciseKind || props.exerciseKind) === "tense-identification";
    computed(() => props.exerciseKind === "mixed" || isTenseIdentification.value);
    ...
    const printIdentifier = computed(() => props.options.showRandomNumber && challengeCode.value ? ui("Défi {code} — fiche {number}", { code: challengeCode.value, number: sheetNumber.value }) : "");
    ...
    function tableQuestionParts(question) {
    ...
    function tableQuestionHeight(question, correction = false) {
      ...
      return Math.max(inclusivePrint.value ? 15 : 11, lineCount * pdfLineHeightMm.value + 5) + Math.max(0, effectiveQuestionSpacingMm.value - 6);
    }
    ...
    const correctionPages = computed(() => paginateByHeight(
      ...
      220,
      (question) => isTableLayout.value ? tableQuestionHeight(question, true) : isIdentificationQuestion(question) ? correctionItemHeight("", printableCorrectionText(question)) * (inclusivePrint.value ? 1.35 : 1) : correctionItemHeight(printableCorrectionLabel(question, props.exerciseKind), printableCorrectionText(question)) * (inclusivePrint.value ? 1.35 : 1)
    ));
    ...
    async function buildPdf() {
      ...
      function drawQuestionTableRow(question, y, correction = false) {
        ...
        const textBaseline = y + height / 2 + fontSize * 25.4 / 72 * 0.28;
...
//# sourceMappingURL=PrintPreview-BUZ1SuUP.mjs.map
