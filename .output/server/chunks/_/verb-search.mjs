function normalizeVerbSearch(value) {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLocaleLowerCase("fr");
}
function matchingVerbs(verbs, query) {
  const needle = normalizeVerbSearch(query);
  if (!needle) return verbs;
  return verbs.filter((verb) => normalizeVerbSearch(verb.infinitif).includes(needle)).sort((left, right) => {
...
export { matchingVerbs as m, normalizeVerbSearch as n };
//# sourceMappingURL=verb-search.mjs.map
