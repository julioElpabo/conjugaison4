...
export default defineEventHandler(async (event) => {
  ...
  await useDatabase().execute(`UPDATE coaches SET slug=?, first_name=?, last_name=?, gender=?, avatar_path=?, description=?, character_id=?,
    theme_color=?, status=?, sort_order=? WHERE id=?`, [profile.slug, profile.firstName, profile.lastName, profile.gender,
    profile.avatarPath, profile.description, profile.characterId, profile.themeColor, profile.status, profile.sortOrder, id])
  return { ok: true }
})
