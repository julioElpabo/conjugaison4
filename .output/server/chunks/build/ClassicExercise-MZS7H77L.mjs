...
function subjunctiveIdentificationExample(question, answer) {
  var _a;
  ...
  const alternative = (_a = question.conjugationConfusions) == null ? void 0 : _a.find(
    (candidate) => normalizeAnswer(candidate.mode) === "subjonctif" && submitted.includes(normalizeAnswer(candidate.tense)) && candidate.answers.some((form) => normalizeAnswer(form.replace(/^qu['’]|^que\s+/iu, "")) === normalizeAnswer(question.consigne))
  );
  if (!alternative) return null;
  const matching = alternative.answers.find(
    (form) => normalizeAnswer(form.replace(/^qu['’]|^que\s+/iu, "")) === normalizeAnswer(question.consigne)
  );
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const currentSubjectMustBeTyped = computed(() => Boolean(
      currentQuestion.value && activeExerciseKind.value === "conjugation" && conjugationRequiresSubjectPronoun(currentQuestion.value)
    ));
    const currentAnswerPlaceholder = computed(() => currentQuestion.value ? conjugationAnswerPlaceholder(currentQuestion.value) : "");
    const providedAnswerPrefix = computed(() => currentQuestion.value && activeExerciseKind.value === "conjugation" ? providedSubjunctiveInputPrefix(currentQuestion.value) : "");
    ...
    const identificationAmbiguityMessage = computed(() => {
      const example = currentQuestion.value && isIdentificationExercise.value ? subjunctiveIdentificationExample(currentQuestion.value, lastIncorrectIdentificationAnswer.value) : null;
    ...
    const incorrectSummaryForms = computed(() => attempts.value.map((attempt) => (attempt.question.exerciseKind ? attempt.question.exerciseKind !== "conjugation" : isIdentificationExercise.value) && attempt.status === "incorrect" ? identificationFormParts(attempt.question) : null));
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        ...
        if (!unref(isFinished) && unref(currentQuestion)) {
          ...
          if (unref(activeExerciseKind) === "tense-identification" || unref(activeExerciseKind) === "mode-identification") {
            _push2(`<p class="question-instruction">${ssrInterpolate(unref(currentQuestion).instruction)}</p>`);
          } else {
            _push2(`<!---->`);
          }
          if (unref(falcMode) && unref(activeExerciseKind) === "conjugation") {
            ...
            _push2(`</form><!--]-->`);
          } else if (unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).complement) {
            ...
            if (unref(retryMessageVisible)) {
              _push2(`<div id="answer-retry" class="answer-retry" role="status" aria-live="polite"><span class="answer-retry__icon" aria-hidden="true">↻</span><div><strong>${ssrInterpolate(unref(identificationAmbiguityMessage) || unref(ui)("Pas encore. Essaie une deuxième fois."))}</strong></div></div>`);
            } else {
              _push2(`<!---->`);
            }
          ...
          if (unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).speech?.questionToken) {
          ...
          if (!unref(falcMode) && !(unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).complement)) {
          ...
          if (unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).speech?.answerToken) {
          ...
          if (unref(missingPronounMessageVisible) && !(unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).complement)) {
            _push2(`<div id="answer-missing-pronoun" class="answer-retry answer-retry--missing-pronoun" role="status" aria-live="polite"><span class="answer-retry__icon" aria-hidden="true">i</span><div><strong>${ssrInterpolate(unref(ui)("Il manque le pronom"))}</strong></div></div>`);
          } else {
            _push2(`<!---->`);
          }
          if (unref(retryMessageVisible) && !(unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).complement)) {
            _push2(`<div id="answer-retry" class="answer-retry" role="status" aria-live="polite"><span class="answer-retry__icon" aria-hidden="true">↻</span><div><strong>${ssrInterpolate(unref(identificationAmbiguityMessage) || unref(ui)("Pas encore. Essaie une deuxième fois."))}</strong></div></div>`);
          } else {
            _push2(`<!---->`);
          }
          if (unref(retryMessageVisible) && unref(retryGuidanceMessages).length && !(unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).complement)) {
            _push2(`<aside class="answer-retry-hint"><strong>${ssrInterpolate(unref(ui)("Un indice pour t’aider"))}</strong><!--[-->`);
            ssrRenderList(unref(retryGuidanceMessages), (message) => {
              _push2(`<p>${ssrInterpolate(message)}</p>`);
            });
          ...
          if (unref(retryMessageVisible) && unref(detectedErrorDetails).length && !(unref(activeExerciseKind) === "conjugation" && unref(currentQuestion).complement)) {
            _push2(ssrRenderComponent(LearnerErrorFeedback, { details: unref(detectedErrorDetails) }, null, _parent));
          } else {
            _push2(`<!---->`);
          }
          ...
          ssrRenderList(unref(attempts), (attempt, index) => {
            _push2(`<tr><td class="result-question-column"><div class="result-question-cell"><div class="result-question-content">`);
            ...
            if (attempt.question.verbeId) {
              _push2(`<button type="button" class="result-consult-verb">${ssrInterpolate(unref(ui)("Consulter"))}</button>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></td><td>${ssrInterpolate(attempt.answer)}</td><td><div class="result-spoken-answers"><!--[-->`);
          ...
          if (!unref(falcMode) && __props.canSaveChallenge) {
...
//# sourceMappingURL=ClassicExercise-MZS7H77L.mjs.map
