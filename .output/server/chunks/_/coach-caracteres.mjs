async function replaceCaractereChildren(connection, id, replies, assignments, rules) {
...
async function deleteCaracterePermanently(connection, id) {
  const [locked] = await connection.execute("SELECT id FROM coach_characters WHERE id=? FOR UPDATE", [id]);
  if (!locked.length) return false;
  await connection.execute("UPDATE coaches SET character_id=NULL WHERE character_id=?", [id]);
  const [result] = await connection.execute("DELETE FROM coach_characters WHERE id=?", [id]);
  if (result.affectedRows !== 1) return false;
  return true;
}

export { deleteCaracterePermanently as d, replaceCaractereChildren as r };
//# sourceMappingURL=coach-caracteres.mjs.map
