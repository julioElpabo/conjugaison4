...
async function createHelpApproach() {
  ...
  try {
    await $fetch('/api/admin/coach-help-approaches', { method: 'POST', body: {
      name, engineKey: 'complete-avec-reponses', status: 'draft', sortOrder: Math.max(0, ...helpApproaches.value.map(item => item.sortOrder)) + 1,
    } })
    newApproachName.value = ''
    ...
    <div v-if="tab === 'caracteres'" class="caractere-workspace">
      ...
      <form v-if="draft" class="caractere-editor" @submit.prevent>
        ...
        <section class="admin-card caractere-panel reaction-media-panel">
          ...
          <p class="admin-muted">Les fréquences s’appliquent une fois la pause écoulée : rarement = 20 %, moitié = 50 %, souvent = 80 %. Le GIF est tiré en premier ; un émoji peut apparaître si aucun GIF n’est choisi. La pause indique l’écart minimal entre les numéros des questions avec un média (0 = aucune pause, 2 = une question intermédiaire).</p>
        ...
        <section class="icon-picker approach-manager" role="dialog" aria-modal="true" aria-labelledby="approach-manager-title">
          ...
          <div class="approach-manager__list">
            <article v-for="approach in approachDrafts" :key="approach.id">
              ...
              <label class="admin-field"><span>Statut</span><select v-model="approach.status"><option value="draft">Brouillon</option><option value="published">Publié</option><option value="disabled">Désactivé</option></select></label>
