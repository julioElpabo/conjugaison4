...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))} data-v-664bdcda><header class="learner-space__hero" data-v-664bdcda><div data-v-664bdcda><h1 data-v-664bdcda>${ssrInterpolate(unref(copy).hello)} ${ssrInterpolate(unref(displayUsername))}</h1></div><nav class="learner-space__hero-actions"${ssrRenderAttr("aria-label", unref(copy).personalSettings)} data-v-664bdcda><button class="${ssrRenderClass({ "is-active": unref(activeTab) === "preferences" })}" type="button" data-v-664bdcda>${ssrInterpolate(unref(copy).preferences)}</button><button class="${ssrRenderClass({ "is-active": unref(activeTab) === "account" })}" type="button" data-v-664bdcda>${ssrInterpolate(unref(copy).account)}</button></nav></header><nav class="learner-tabs" style="${ssrRenderStyle(unref(stickyTabsStyle))}"${ssrRenderAttr("aria-label", unref(copy).spaceSections)} data-v-664bdcda><button class="${ssrRenderClass([{ "is-active": unref(activeTab) === "history" }, "learner-tabs__primary"])}" type="button" data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>✦</span> ${ssrInterpolate(unref(copy).history)}</button><button class="${ssrRenderClass({ "is-active": unref(activeTab) === "progress" })}" type="button" data-v-664bdcda>${ssrInterpolate(unref(copy).commonErrors)}</button></nav>`);
      if (unref(activeTab) === "challenges" || unref(activeTab) === "history") {
        _push(`<section class="learner-panel" aria-labelledby="challenges-title" data-v-664bdcda><div class="learner-panel__heading" data-v-664bdcda><div data-v-664bdcda><p class="learner-eyebrow" data-v-664bdcda>${ssrInterpolate(unref(activeTab) === "history" ? unref(copy).findActivities : unref(copy).resumeAndConsolidate)}</p><h2 id="challenges-title" data-v-664bdcda>${ssrInterpolate(unref(activeTab) === "history" ? unref(copy).history : unref(copy).improve)}</h2></div></div><aside class="learner-section-intro" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(activeTab) === "history" ? unref(copy).reviewJourney : unref(copy).chooseTraining)}</strong>`);
        if (unref(activeTab) === "history") {
          _push(`<p data-v-664bdcda>${ssrInterpolate(unref(copy).historyIntro)}</p>`);
        } else {
          _push(`<p data-v-664bdcda>${ssrInterpolate(unref(copy).improveIntro)}</p>`);
        }
        _push(`</aside>`);
        if (unref(activeTab) === "history") {
          _push(`<!--[-->`);
          if (unref(dashboardPending)) {
            _push(`<p class="learner-empty" data-v-664bdcda>${ssrInterpolate(unref(copy).loadingChallenges)}</p>`);
          } else if (unref(dashboard)?.challenges.length) {
            _push(`<!--[--><ol class="challenge-history"${ssrRenderAttr("aria-label", unref(copy).challengeHistory)} data-v-664bdcda><!--[-->`);
            ssrRenderList(unref(challengeDays), (day) => {
              _push(`<li class="challenge-history__day" data-v-664bdcda><time class="challenge-history__date"${ssrRenderAttr("datetime", day.key)} data-v-664bdcda>${ssrInterpolate(day.label)}</time><ol class="challenge-history__day-list" data-v-664bdcda><!--[-->`);
              ssrRenderList(day.challenges, (challenge, challengeIndex) => {
                ...
                })}" data-v-664bdcda><span class="challenge-history__dot" aria-hidden="true" data-v-664bdcda></span><article class="${ssrRenderClass([{ "challenge-card--perfect": challengeIsComplete(challenge) && challenge.incorrectCount === 0 }, "challenge-card"])}" data-v-664bdcda><div class="challenge-card__top" data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(challenge.label)}</h3><span data-v-664bdcda>${ssrInterpolate(formattedChallengeTime(challenge.lastActivityAt))}</span></div>`);
                if (challenge.description && challenge.incorrectCount > 0) {
                  _push(`<div class="challenge-card__description" data-v-664bdcda><p data-v-664bdcda>${ssrInterpolate(challenge.description)}</p></div>`);
                } else {
                  _push(`<!---->`);
                }
                _push(`<div class="challenge-card__question-progress" role="img"${ssrRenderAttr("aria-label", challengeProgressLabel(challenge))} data-v-664bdcda><!--[-->`);
                ssrRenderList(challenge.challenge.questionCount, (index) => {
                  ...
                  })}" data-v-664bdcda></span>`);
                });
                _push(`<!--]--></div><div class="${ssrRenderClass([{
                  "has-single-action": __props.readOnly || challengeIsComplete(challenge)
                }, "challenge-card__primary-actions"])}" data-v-664bdcda>`);
                if (!__props.readOnly && !challengeIsComplete(challenge)) {
                  _push(`<div class="challenge-finish" data-v-664bdcda><button type="button" class="challenge-card__finish-session"${ssrRenderAttr("aria-expanded", unref(finishMenuChallengeId) === challenge.id)} data-v-664bdcda><span class="review-button__scope-icon" aria-hidden="true" data-v-664bdcda><svg viewBox="0 0 24 24" data-v-664bdcda><path d="M8 5v14l11-7z" data-v-664bdcda></path></svg></span><span class="review-button__scope-copy" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(ui)("Terminer la séance"))}</strong><small data-v-664bdcda>${ssrInterpolate(unref(ui)("Reprendre à la prochaine question"))}</small></span></button>`);
                  if (unref(finishMenuChallengeId) === challenge.id) {
                    _push(`<div class="challenge-presentation-menu" role="group"${ssrRenderAttr("aria-label", unref(copy).choosePresentation)} data-v-664bdcda><button class="action-button action-button--primary" type="button"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""} data-v-664bdcda><span class="action-button__icon" aria-hidden="true" data-v-664bdcda>●</span><span data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).classic)}</strong></span></button><button class="action-button action-button--chat" type="button"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""} data-v-664bdcda><span class="action-button__icon" aria-hidden="true" data-v-664bdcda>`);
                    if (unref(randomCoachAvatar)) {
                      _push(`<img${ssrRenderAttr("src", unref(randomCoachAvatar))} alt="" data-v-664bdcda>`);
                    } else {
                      _push(`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" data-v-664bdcda><circle cx="12" cy="8" r="4" data-v-664bdcda></circle><path d="M4.5 21a7.5 7.5 0 0 1 15 0" data-v-664bdcda></path></svg>`);
                    }
                    _push(`</span><span data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).withCoach)}</strong></span></button></div>`);
                  } else {
                    _push(`<!---->`);
                  }
                  _push(`</div>`);
                ...
                _push(`<button type="button" class="review-button review-button--summary"${ssrIncludeBooleanAttr(unref(historySummaryPendingId) === challenge.id) ? " disabled" : ""} data-v-664bdcda><span class="review-button__scope-icon" aria-hidden="true" data-v-664bdcda><svg viewBox="0 0 24 24" data-v-664bdcda><path d="M6 3h9l3 3v15H6zM9 10h6M9 14h6M9 18h4" data-v-664bdcda></path></svg></span><span class="review-button__scope-copy" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).viewSummary)}</strong><small data-v-664bdcda>${ssrInterpolate(unref(copy).summaryHint)}</small></span></button></div>`);
                if (unref(historySummaryError)?.challengeId === challenge.id) {
                  _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(historySummaryError).message)}</p>`);
                } else {
                  _push(`<!---->`);
                }
                if (!__props.readOnly) {
                  _push(`<div class="challenge-work" data-v-664bdcda>`);
                  if (challengeIsComplete(challenge)) {
                    _push(`<div class="challenge-work__group" data-v-664bdcda><header class="challenge-work__heading" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).retrainChallenge)}</strong></header><div data-v-664bdcda><button type="button" class="review-button review-button--choice"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""}${ssrRenderAttr("title", challenge.exactQuestions.length ? unref(copy).sameDraw : unref(copy).oldDraw)}${ssrRenderAttr("aria-expanded", unref(workMenuFingerprint) === `${challenge.id}-same`)} data-v-664bdcda><svg viewBox="0 0 24 24" aria-hidden="true" data-v-664bdcda><path d="M7 7h10M7 12h10M7 17h10M4 7h.01M4 12h.01M4 17h.01" data-v-664bdcda></path></svg><span data-v-664bdcda>${ssrInterpolate(unref(copy).sameOrder)}</span></button><button type="button" class="review-button review-button--choice"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""}${ssrRenderAttr("aria-expanded", unref(workMenuFingerprint) === `${challenge.id}-random`)} data-v-664bdcda><svg viewBox="0 0 24 24" aria-hidden="true" data-v-664bdcda><path d="M4 7h3c4 0 6 10 10 10h3M17 4l3 3-3 3M4 17h3c1.7 0 3-1.7 4.2-3.7M16 7h4M17 14l3 3-3 3" data-v-664bdcda></path></svg><span data-v-664bdcda>${ssrInterpolate(unref(copy).randomOrder)}</span></button></div></div>`);
                  } else {
                    _push(`<!---->`);
                  }
                  if (challenge.unresolvedCount > 0) {
                    _push(`<div class="challenge-work__group" data-v-664bdcda><header class="challenge-work__heading" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).retrainErrors)}</strong></header><div data-v-664bdcda>`);
                    if (challenge.unresolvedCount > 0) {
                      _push(`<button type="button" class="review-button review-button--error-scope"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""}${ssrRenderAttr("aria-expanded", unref(workMenuFingerprint) === `${challenge.id}-incorrect`)} data-v-664bdcda><span class="review-button__scope-icon" aria-hidden="true" data-v-664bdcda><svg viewBox="0 0 24 24" data-v-664bdcda><rect x="5" y="3" width="14" height="18" rx="3" data-v-664bdcda></rect><path d="M9 8h6M9 12h6M9 16h4" data-v-664bdcda></path></svg></span><span class="review-button__scope-copy" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).errorsThisSession)}</strong><small data-v-664bdcda>${ssrInterpolate(challengeErrorsLabel(challenge.unresolvedCount))}</small></span></button>`);
                    } else {
                      _push(`<!---->`);
                    }
                    if (challenge.allUnresolvedCount > 0) {
                      _push(`<button type="button" class="review-button review-button--error-scope"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""}${ssrRenderAttr("aria-expanded", unref(workMenuFingerprint) === `${challenge.id}-all-incorrect`)} data-v-664bdcda><span class="review-button__scope-icon" aria-hidden="true" data-v-664bdcda><svg viewBox="0 0 24 24" data-v-664bdcda><rect x="7" y="3" width="12" height="16" rx="3" data-v-664bdcda></rect><path d="M5 7H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-1M10 8h6M10 12h6" data-v-664bdcda></path></svg></span><span class="review-button__scope-copy" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).errorsWholeChallenge)}</strong><small data-v-664bdcda>${ssrInterpolate(challengeErrorsLabel(challenge.allUnresolvedCount))}</small></span></button>`);
                    } else {
                      _push(`<!---->`);
                    }
                    _push(`</div></div>`);
                  ...
                  if (unref(selectedWork)?.challenge.id === challenge.id && unref(workMenuFingerprint)) {
                    _push(`<div class="challenge-presentation-menu" style="${ssrRenderStyle({ left: `${unref(workMenuLeft)}px` })}" role="group"${ssrRenderAttr("aria-label", unref(copy).choosePresentation)} data-v-664bdcda><button class="action-button action-button--primary" type="button"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""} data-v-664bdcda><span class="action-button__icon" aria-hidden="true" data-v-664bdcda>●</span><span data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).classic)}</strong></span></button><button class="action-button action-button--chat" type="button"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""} data-v-664bdcda><span class="action-button__icon" aria-hidden="true" data-v-664bdcda>`);
                    if (unref(randomCoachAvatar)) {
                      _push(`<img${ssrRenderAttr("src", unref(randomCoachAvatar))} alt="" data-v-664bdcda>`);
                    } else {
                      _push(`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" data-v-664bdcda><circle cx="12" cy="8" r="4" data-v-664bdcda></circle><path d="M4.5 21a7.5 7.5 0 0 1 15 0" data-v-664bdcda></path></svg>`);
            ...
            if (unref(challengeStartError)) {
              _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(challengeStartError))}</p>`);
            } else {
              _push(`<!---->`);
            }
            if (unref(historyHasMore)) {
              _push(`<div class="challenge-loader-sentinel" aria-hidden="true" data-v-664bdcda></div>`);
            } else {
              _push(`<!---->`);
            }
            _push(`<!--]-->`);
          } else {
            _push(`<div class="learner-empty" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).noChallenge)}</strong><span data-v-664bdcda>${ssrInterpolate(unref(copy).futureAnswers)}</span></div>`);
          }
          _push(`<!--]-->`);
        } else {
          _push(`<!--[-->`);
          if (unref(challengeTrainingsError)) {
            _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(challengeTrainingsError))}</p>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(challengeTrainingsPending) && !unref(challengeTrainings).length) {
            _push(`<p class="learner-empty" data-v-664bdcda>${ssrInterpolate(unref(copy).groupingTrainings)}</p>`);
          } else if (unref(challengeTrainings).length) {
            _push(`<div class="challenge-training-layout" data-v-664bdcda><nav class="challenge-training-list"${ssrRenderAttr("aria-label", unref(copy).trainedChallenges)} data-v-664bdcda><!--[-->`);
            ssrRenderList(unref(challengeTrainings), (training) => {
              _push(`<button type="button" class="${ssrRenderClass({ "is-active": unref(selectedTrainingFingerprint) === training.fingerprint })}"${ssrRenderAttr("aria-current", unref(selectedTrainingFingerprint) === training.fingerprint ? "true" : void 0)} data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(training.label)}</span><small data-v-664bdcda>${ssrInterpolate(trainingDateLabel(training.lastTrainedAt))}</small><b data-v-664bdcda>${ssrInterpolate(trainingCountLabel(training.sessionCount))} · ${ssrInterpolate(training.latestSuccessPercent)}% </b></button>`);
            });
            _push(`<!--]--></nav><section class="challenge-training-analysis" aria-live="polite" data-v-664bdcda>`);
            if (unref(selectedTraining)) {
              _push(`<header data-v-664bdcda><div data-v-664bdcda><p data-v-664bdcda>${ssrInterpolate(unref(copy).successEvolution)}</p><h3 data-v-664bdcda>${ssrInterpolate(unref(selectedTraining).label)}</h3></div></header>`);
            } else {
              _push(`<!---->`);
            }
            if (unref(selectedTrainingProgressError)) {
              _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(selectedTrainingProgressError))}</p>`);
            } else if (unref(selectedTrainingProgressPending)) {
              _push(`<div class="challenge-training-state" data-v-664bdcda>${ssrInterpolate(unref(copy).calculatingProgress)}</div>`);
            } else if (unref(selectedTrainingProgress)?.points.length) {
              _push(`<!--[--><div class="challenge-training-chart" data-v-664bdcda><div class="challenge-training-chart__plot" data-v-664bdcda><svg viewBox="0 0 640 190" role="img"${ssrRenderAttr("aria-label", successEvolutionLabel(unref(selectedTraining)?.label || ""))} data-v-664bdcda><defs data-v-664bdcda><linearGradient id="training-success-gradient" x1="0" y1="156" x2="0" y2="16" gradientUnits="userSpaceOnUse" data-v-664bdcda><stop offset="0%" stop-color="#ef6760" data-v-664bdcda></stop><stop offset="50%" stop-color="#d6a33e" data-v-664bdcda></stop><stop offset="100%" stop-color="#3aa66f" data-v-664bdcda></stop></linearGradient></defs><g class="challenge-training-chart__grid" data-v-664bdcda><line x1="42" x2="624" y1="16" y2="16" data-v-664bdcda></line><line x1="42" x2="624" y1="86" y2="86" data-v-664bdcda></line><line x1="42" x2="624" y1="156" y2="156" data-v-664bdcda></line></g>`);
              if (unref(trainingChartCoordinates).length > 1) {
                _push(`<polyline${ssrRenderAttr("points", unref(trainingChartPolyline))} data-v-664bdcda></polyline>`);
              } else {
                _push(`<!---->`);
              }
              _push(`<!--[-->`);
              ssrRenderList(unref(trainingChartCoordinates), (coordinate) => {
                _push(`<circle${ssrRenderAttr("cx", coordinate.x)}${ssrRenderAttr("cy", coordinate.y)} style="${ssrRenderStyle({ stroke: trainingSuccessColor(coordinate.point.successPercent) })}" class="${ssrRenderClass({ "is-active": unref(hoveredTrainingPointId) === coordinate.point.id })}" role="button" tabindex="0"${ssrRenderAttr("aria-label", trainingPointLabel(coordinate.point))}${ssrRenderAttr("r", trainingPointRadius(coordinate.point.totalCount) + (unref(hoveredTrainingPointId) === coordinate.point.id ? 2 : 0))} data-v-664bdcda></circle>`);
              });
              _push(`<!--]--><!--[-->`);
              ssrRenderList(unref(trainingChartCoordinates), (coordinate) => {
                _push(`<text class="challenge-training-chart__point-count"${ssrRenderAttr("x", coordinate.x)}${ssrRenderAttr("y", coordinate.y)} aria-hidden="true" data-v-664bdcda>${ssrInterpolate(coordinate.point.totalCount)}</text>`);
              });
              _push(`<!--]--></svg>`);
              if (unref(hoveredTrainingCoordinate)) {
                ...
                })}" role="tooltip" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(successPercentLabel(unref(hoveredTrainingCoordinate).point.successPercent))}</strong><span data-v-664bdcda>${ssrInterpolate(trainingDateLabel(unref(hoveredTrainingCoordinate).point.occurredAt, true))}</span><small data-v-664bdcda>${ssrInterpolate(unref(hoveredTrainingCoordinate).point.correctCount)}/${ssrInterpolate(unref(hoveredTrainingCoordinate).point.totalCount)} ${ssrInterpolate(responseSummaryLabel(unref(hoveredTrainingCoordinate).point.correctCount, unref(hoveredTrainingCoordinate).point.incorrectCount))}</small><b data-v-664bdcda>${ssrInterpolate(unref(copy).clickSession)}</b></div>`);
              } else {
                _push(`<!---->`);
              }
              _push(`<div class="challenge-training-chart__rate-axis" aria-hidden="true" data-v-664bdcda><span class="is-high" data-v-664bdcda>100%</span><span class="is-middle" data-v-664bdcda>50%</span><span class="is-low" data-v-664bdcda>0%</span></div><div class="challenge-training-chart__date-axis" aria-hidden="true" data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(trainingDateLabel(unref(selectedTrainingProgress).points[0].occurredAt))}</span><span data-v-664bdcda>${ssrInterpolate(trainingDateLabel(unref(selectedTrainingProgress).points.at(-1).occurredAt))}</span></div></div><footer data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(occurrenceCountLabel(unref(selectedTrainingProgress).points.length))}</span></footer></div><section class="challenge-training-achievement"${ssrRenderAttr("aria-label", unref(copy).achievementLabel)} data-v-664bdcda><div data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(unref(copy).bestResult)}</span><strong data-v-664bdcda><span class="challenge-training-achievement__rate" data-v-664bdcda>${ssrInterpolate(successPercentLabel(unref(selectedTrainingProgress).achievement.bestSuccessPercent))}</span><small data-v-664bdcda>${ssrInterpolate(unref(selectedTrainingProgress).achievement.bestAnsweredQuestionCount)} ${ssrInterpolate(questionsOutOfLabel(unref(selectedTrainingProgress).achievement.bestAnsweredQuestionCount, unref(selectedTrainingProgress).achievement.questionCount))}</small></strong></div><div class="${ssrRenderClass([{ "is-achieved": unref(selectedTrainingProgress).achievement.completedWithoutError }, "challenge-training-achievement__complete"])}" data-v-664bdcda><span class="challenge-training-achievement__check" role="img"${ssrRenderAttr("aria-label", unref(selectedTrainingProgress).achievement.completedWithoutError ? unref(copy).completeSuccess : unref(copy).notCompleteSuccess)}${ssrRenderAttr("aria-disabled", !unref(selectedTrainingProgress).achievement.completedWithoutError)} data-v-664bdcda>✓</span><p data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).exerciseCompleted)}</strong><span data-v-664bdcda>(${ssrInterpolate(questionCountLabel(unref(selectedTrainingProgress).achievement.questionCount))})</span></p></div></section>`);
              if (!__props.readOnly) {
                _push(`<div class="challenge-work challenge-training-all-work" data-v-664bdcda><button type="button" class="challenge-training-all-work__button"${ssrIncludeBooleanAttr(!unref(allTrainingErrorQuestions).length || Boolean(unref(challengeStarting))) ? " disabled" : ""}${ssrRenderAttr("aria-expanded", unref(workMenuFingerprint) === "training-all")} data-v-664bdcda>${ssrInterpolate(unref(copy).practiseChallengeErrors)}</button>`);
              ...
              _push(`<section class="challenge-training-sessions" aria-labelledby="training-sessions-title" data-v-664bdcda><header data-v-664bdcda><div data-v-664bdcda><p data-v-664bdcda>${ssrInterpolate(unref(copy).newestFirst)}</p><h4 id="training-sessions-title" data-v-664bdcda>${ssrInterpolate(unref(copy).errorsBySession)}</h4></div><span data-v-664bdcda>${ssrInterpolate(occurrenceCountLabel(unref(trainingSessions).length))}</span></header><div class="challenge-training-sessions__list" data-v-664bdcda><!--[-->`);
              ssrRenderList(unref(trainingSessions), (session) => {
                _push(`<article${ssrRenderAttr("id", trainingSessionId(session.id))} tabindex="-1" data-v-664bdcda>`);
                if (session.errors.length && !__props.readOnly) {
                  _push(`<div class="challenge-work challenge-training-session__work" data-v-664bdcda><button type="button"${ssrIncludeBooleanAttr(Boolean(unref(challengeStarting))) ? " disabled" : ""}${ssrRenderAttr("aria-expanded", unref(workMenuFingerprint) === `training-session-work-${session.id}`)} data-v-664bdcda>${ssrInterpolate(trainQuestionsLabel(session.errors.length))}</button>`);
                ...
                _push(`<header data-v-664bdcda><div data-v-664bdcda>`);
                if (session.title) {
                  _push(`<h5 data-v-664bdcda>${ssrInterpolate(localizedTrainingReportTitle(session.title))}</h5>`);
                } else {
                  _push(`<!---->`);
                }
                _push(`<time${ssrRenderAttr("datetime", session.occurredAt)} data-v-664bdcda>${ssrInterpolate(trainingDateLabel(session.occurredAt, true))}</time><span data-v-664bdcda>${ssrInterpolate(sessionResultLabel(session.correctCount, session.totalCount))}</span></div><strong style="${ssrRenderStyle({ color: trainingSuccessColor(session.successPercent) })}" data-v-664bdcda>${ssrInterpolate(session.successPercent)}% </strong></header>`);
                if (session.errors.length) {
                  _push(`<ol data-v-664bdcda><!--[-->`);
                  ssrRenderList(session.errors, (error) => {
                    _push(`<li data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate([error.infinitive, error.mode, error.tense, error.person].filter(Boolean).join(" · "))}</span><div data-v-664bdcda><span class="challenge-training-error__answer" data-v-664bdcda>${ssrInterpolate(error.learnerAnswer)}</span><span aria-hidden="true" data-v-664bdcda>→</span><strong data-v-664bdcda>${ssrInterpolate(error.expectedAnswers.join(` ${unref(ui)("ou")} `) || unref(copy).unavailableAnswer)}</strong></div><!--[-->`);
                    ssrRenderList(error.explanations, (explanation) => {
                      _push(`<p class="challenge-training-error__explanation" data-v-664bdcda>${ssrInterpolate(explanation)}</p>`);
                    });
                    _push(`<!--]--></li>`);
                  });
                  _push(`<!--]--></ol>`);
                } else {
                  _push(`<p class="challenge-training-session__perfect" data-v-664bdcda>${ssrInterpolate(unref(copy).noSessionError)}</p>`);
                }
                _push(`</article>`);
              });
              _push(`<!--]--></div></section><!--]-->`);
            } else {
              _push(`<div class="challenge-training-state" data-v-664bdcda>${ssrInterpolate(unref(copy).noUsableOccurrence)}</div>`);
            }
            _push(`</section></div>`);
          } else if (!unref(challengeTrainingsPending)) {
            _push(`<div class="learner-empty" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).noTraining)}</strong><span data-v-664bdcda>${ssrInterpolate(unref(copy).noTrainingHint)}</span></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<!--]-->`);
        ...
        _push(`<section class="learner-panel" aria-labelledby="progress-title" data-v-664bdcda><div class="learner-panel__heading learner-panel__heading--progress" data-v-664bdcda><div data-v-664bdcda><p class="learner-eyebrow" data-v-664bdcda>${ssrInterpolate(unref(copy).measureProgress)}</p><h2 id="progress-title" data-v-664bdcda>${ssrInterpolate(unref(copy).commonErrors)}</h2></div><div class="progress-explanation" data-v-664bdcda><button type="button" class="progress-explanation__button"${ssrRenderAttr("aria-label", unref(copy).progressCalculation)} aria-controls="progress-explanation-tooltip"${ssrRenderAttr("aria-expanded", unref(progressExplanationOpen))}${ssrRenderAttr("aria-describedby", unref(progressExplanationOpen) ? "progress-explanation-tooltip" : void 0)} data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>i</span></button>`);
        if (unref(progressExplanationOpen)) {
          _push(`<div id="progress-explanation-tooltip" class="progress-explanation__tooltip" role="tooltip" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).compareRates)}</strong><p data-v-664bdcda>${ssrInterpolate(text("progressExplanation", { count: unref(learnerProgress)?.opportunityWindow || 10 }))}</p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div><aside class="learner-section-intro" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).errorsOverTime)}</strong><p data-v-664bdcda>${ssrInterpolate(unref(copy).errorsIntro)}</p></aside>`);
        if (unref(learnerProgressError)) {
          _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(learnerProgressError))}</p>`);
        } else if (unref(learnerProgressPending) && !unref(learnerProgress)) {
          _push(`<p class="learner-empty" data-v-664bdcda>${ssrInterpolate(unref(copy).analysingProgress)}</p>`);
        } else if (unref(learnerProgress)?.cards.length) {
          _push(`<div class="error-progress-list" data-v-664bdcda><!--[-->`);
          ssrRenderList(unref(learnerProgress).cards, (card, cardIndex) => {
            ...
            ], "error-progress-card"])}" tabindex="-1" data-v-664bdcda><header class="error-progress-card__heading" data-v-664bdcda><div data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(progressCardDomain(card))}</span><h3 data-v-664bdcda>${ssrInterpolate(progressCardLabel(card))}</h3><p data-v-664bdcda>${ssrInterpolate(progressCardAdvice(card))}</p></div><div class="error-progress-card__badges" data-v-664bdcda><div class="error-progress-card__affected" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(card.affectedChallengeCount)}</strong><span data-v-664bdcda>${ssrInterpolate(affectedChallengesLabel(card.affectedChallengeCount))}</span></div><div class="error-progress-card__rate" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(progressCurrentRate(card))}%</strong><span data-v-664bdcda>${ssrInterpolate(unref(copy).errorRate)}</span></div></div></header><div class="error-progress-card__status" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(progressTrendLabel(card))}</strong><span class="${ssrRenderClass({ "is-warning": card.isStale })}" data-v-664bdcda>${ssrInterpolate(progressLastTestLabel(card))}</span></div>`);
            if (card.points.length) {
              _push(`<div class="error-progress-chart" data-v-664bdcda><div class="error-progress-chart__plot" data-v-664bdcda><svg viewBox="0 0 640 190" role="img"${ssrRenderAttr("aria-label", errorEvolutionLabel(progressCardLabel(card)))} data-v-664bdcda><defs data-v-664bdcda><linearGradient${ssrRenderAttr("id", progressGradientId(card))} x1="0" y1="156" x2="0" y2="16" gradientUnits="userSpaceOnUse" data-v-664bdcda><stop offset="0%" stop-color="#3aa66f" data-v-664bdcda></stop><stop offset="10%" stop-color="#d6a33e" data-v-664bdcda></stop><stop offset="20%" stop-color="#ef6760" data-v-664bdcda></stop><stop offset="100%" stop-color="#ef6760" data-v-664bdcda></stop></linearGradient></defs><g class="error-progress-chart__grid" data-v-664bdcda><line x1="42" x2="624" y1="16" y2="16" data-v-664bdcda></line><line x1="42" x2="624" y1="86" y2="86" data-v-664bdcda></line><line x1="42" x2="624" y1="156" y2="156" data-v-664bdcda></line></g><!--[-->`);
              ssrRenderList(progressChartSegments(card), (segment, segmentIndex) => {
                _push(`<polyline${ssrRenderAttr("points", segment.map((coordinate) => `${coordinate.x},${coordinate.y}`).join(" "))} style="${ssrRenderStyle({ stroke: `url(#${progressGradientId(card)})` })}" data-v-664bdcda></polyline>`);
              });
              _push(`<!--]--><!--[-->`);
              ssrRenderList(progressChartCoordinates(card), (coordinate, coordinateIndex) => {
                _push(`<circle${ssrRenderAttr("cx", coordinate.x)}${ssrRenderAttr("cy", coordinate.y)} style="${ssrRenderStyle({ stroke: progressMetricColor(coordinate.rate) })}" r="5" data-v-664bdcda><title data-v-664bdcda>${ssrInterpolate(progressDateLabel(coordinate.point.date))} : ${ssrInterpolate(chartPointLabel(coordinate.rate, coordinate.point.errors, coordinate.point.opportunities))}</title></circle>`);
              });
              _push(`<!--]--></svg><div class="error-progress-chart__rate-axis is-errors" aria-hidden="true" data-v-664bdcda><span class="is-high" data-v-664bdcda>100%</span><span class="is-middle" data-v-664bdcda>50%</span><span class="is-low" data-v-664bdcda>0%</span></div><div class="error-progress-chart__date-axis" aria-hidden="true" data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(progressDateLabel(card.points[0].date))}</span><span data-v-664bdcda>${ssrInterpolate(progressDateLabel(card.points.at(-1).date))}</span></div></div><div class="error-progress-chart__legend" data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(unref(copy).fewerErrorsBelow)}</span><span data-v-664bdcda>${ssrInterpolate(totalOpportunitiesLabel(card.totalOpportunities))}</span></div></div>`);
            } else {
              _push(`<div class="error-progress-card__insufficient" data-v-664bdcda>${ssrInterpolate(text("reliableCurve", { count: unref(learnerProgress).minimumEvidence }))}</div>`);
            }
            if (card.examples.length) {
              _push(`<details class="error-progress-examples"${ssrIncludeBooleanAttr(cardIndex === 0) ? " open" : ""} data-v-664bdcda><summary data-v-664bdcda><span class="error-progress-examples__label" data-v-664bdcda><span class="error-progress-examples__chevron" aria-hidden="true" data-v-664bdcda></span> ${ssrInterpolate(progressExamplesLabel(card))}</span></summary><ol data-v-664bdcda><!--[-->`);
              ssrRenderList(card.examples, (example) => {
                _push(`<li data-v-664bdcda><dl data-v-664bdcda><div data-v-664bdcda><dt data-v-664bdcda>${ssrInterpolate(unref(copy).question)}</dt><dd data-v-664bdcda>${ssrInterpolate(example.question)}</dd></div><!--[-->`);
                ssrRenderList([learnerErrorComparison(example)], (comparison) => {
                  _push(`<!--[-->`);
                  if (comparison) {
                    _push(`<div class="error-progress-comparison-row error-progress-comparison-row--learner" data-v-664bdcda><dt data-v-664bdcda>${ssrInterpolate(unref(ui)("Ta réponse"))}</dt><dd class="error-progress-comparison__answer error-progress-comparison__answer--learner" data-v-664bdcda><!--[-->`);
                    ssrRenderList(comparison.learnerParts, (part, partIndex) => {
                      _push(`<span class="${ssrRenderClass(`error-progress-comparison__part--${part.kind}`)}" data-v-664bdcda>${ssrInterpolate(part.text)}</span>`);
                    });
                    _push(`<!--]--></dd></div>`);
                  } else {
                    _push(`<!---->`);
                  ...
                  if (comparison) {
                    _push(`<div class="error-progress-comparison-row error-progress-comparison-row--expected" data-v-664bdcda><dt data-v-664bdcda>${ssrInterpolate(unref(copy).correction)}</dt><dd class="error-progress-comparison__answer error-progress-comparison__answer--expected" data-v-664bdcda><!--[-->`);
                    ssrRenderList(comparison.expectedParts, (part, partIndex) => {
                      _push(`<span class="${ssrRenderClass(`error-progress-comparison__part--${part.kind}`)}" data-v-664bdcda>${ssrInterpolate(part.text)}</span>`);
                    });
                    _push(`<!--]--></dd></div>`);
                  } else {
                    _push(`<!--[--><div data-v-664bdcda><dt data-v-664bdcda>${ssrInterpolate(unref(copy).yourError)}</dt><dd class="error-progress-examples__wrong" data-v-664bdcda>${ssrInterpolate(example.learnerAnswer || unref(copy).noAnswer)}</dd></div><div data-v-664bdcda><dt data-v-664bdcda>${ssrInterpolate(unref(copy).correction)}</dt><dd class="error-progress-examples__correct" data-v-664bdcda>${ssrInterpolate(example.expectedAnswers.join(` ${unref(ui)("ou")} `) || unref(copy).unavailableAnswer)}</dd></div><!--]-->`);
                  }
                  _push(`<!--]-->`);
                });
                _push(`<!--]--><div data-v-664bdcda><dt data-v-664bdcda>${ssrInterpolate(unref(copy).reason)}</dt><dd data-v-664bdcda>`);
              ...
              if (card.hasMoreExamples) {
                _push(`<div class="error-progress-examples__more" data-v-664bdcda><button type="button"${ssrIncludeBooleanAttr(unref(progressExamplesPendingCode) === card.code) ? " disabled" : ""} data-v-664bdcda>${ssrInterpolate(unref(progressExamplesPendingCode) === card.code ? unref(copy).loadingMoreErrors : unref(copy).seeMore)}</button></div>`);
              } else {
                _push(`<!---->`);
              }
              _push(`</details>`);
            ...
            if (!__props.readOnly && errorChallengeIsAvailable(card)) {
              _push(`<button type="button" class="error-progress-card__challenge"${ssrIncludeBooleanAttr(Boolean(unref(errorChallengePendingCode)) || Boolean(unref(challengeStarting))) ? " disabled" : ""} data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>▶</span><strong data-v-664bdcda>${ssrInterpolate(unref(errorChallengePendingCode) === card.code ? unref(copy).preparingErrorChallenge : errorChallengeButtonLabel(card))}</strong><small data-v-664bdcda>${ssrInterpolate(unref(copy).tenQuestionChallenge)}</small></button>`);
            } else {
              _push(`<!---->`);
            }
            if (unref(errorChallengeErrorCode) === card.code) {
              _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(copy).prepareError)}</p>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</article>`);
          ...
          _push(`<div class="learner-empty" data-v-664bdcda><strong data-v-664bdcda>${ssrInterpolate(unref(copy).insufficientData)}</strong><span data-v-664bdcda>${ssrInterpolate(unref(copy).futureErrorTypes)}</span></div>`);
        }
        _push(`</section>`);
      } else if (unref(activeTab) === "preferences") {
        _push(`<section class="learner-panel" aria-labelledby="preferences-title" data-v-664bdcda><div class="learner-panel__heading" data-v-664bdcda><div data-v-664bdcda><p class="learner-eyebrow" data-v-664bdcda>${ssrInterpolate(unref(copy).adaptInterface)}</p><h2 id="preferences-title" data-v-664bdcda>${ssrInterpolate(unref(copy).preferences)}</h2></div>`);
        if (unref(preferencesSaved)) {
          _push(`<span class="preferences-saved" data-v-664bdcda>${ssrInterpolate(unref(copy).saved)}</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
        if (unref(preferencesError)) {
          _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(preferencesError))}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="preference-block" data-v-664bdcda><div data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(unref(copy).language)}</h3><p data-v-664bdcda>${ssrInterpolate(unref(copy).languageHint)}</p></div><div class="locale-choices" role="group"${ssrRenderAttr("aria-label", unref(copy).preferredLanguage)} data-v-664bdcda><!--[-->`);
        ssrRenderList(unref(localeOptions), (option) => {
          _push(`<button type="button" class="${ssrRenderClass({ "is-active": unref(preferredLocale) === option.value })}"${ssrIncludeBooleanAttr(__props.readOnly || unref(preferencesSaving)) ? " disabled" : ""} data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>${ssrInterpolate(option.flag)}</span>${ssrInterpolate(option.label)}</button>`);
        });
        _push(`<!--]--></div></div><div class="preference-block" data-v-664bdcda><div data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(unref(copy).appearance)}</h3><p data-v-664bdcda>${ssrInterpolate(unref(copy).appearanceHint)}</p></div><div class="theme-choices" role="group"${ssrRenderAttr("aria-label", unref(copy).preferredAppearance)} data-v-664bdcda><button type="button" class="${ssrRenderClass({ "is-active": unref(preferredTheme) === "light" })}"${ssrIncludeBooleanAttr(__props.readOnly || unref(preferencesSaving)) ? " disabled" : ""} data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>☀️</span> ${ssrInterpolate(unref(copy).light)}</button><button type="button" class="${ssrRenderClass({ "is-active": unref(preferredTheme) === "dark" })}"${ssrIncludeBooleanAttr(__props.readOnly || unref(preferencesSaving)) ? " disabled" : ""} data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>🌙</span> ${ssrInterpolate(unref(copy).dark)}</button></div></div></section>`);
      } else {
        _push(`<section class="learner-panel account-panel" aria-labelledby="account-title" data-v-664bdcda><div class="learner-panel__heading" data-v-664bdcda><div data-v-664bdcda><p class="learner-eyebrow" data-v-664bdcda>${ssrInterpolate(unref(copy).dataSimply)}</p><h2 id="account-title" data-v-664bdcda>${ssrInterpolate(unref(copy).account)}</h2></div></div><div class="account-privacy" data-v-664bdcda><span aria-hidden="true" data-v-664bdcda>◌</span><div data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(unref(copy).privateAccount)}</h3><p data-v-664bdcda>${ssrInterpolate(text("privacy", { username: unref(displayUsername) }))}</p></div></div>`);
        if (unref(resultsDeleted)) {
          _push(`<p class="account-success" role="status" data-v-664bdcda>${ssrInterpolate(unref(copy).resultsDeleted)}</p>`);
        } else {
          _push(`<!---->`);
        }
        if (!__props.readOnly) {
          _push(`<form id="change-password" class="account-password" data-v-664bdcda><div data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(unref(copy).changePassword)}</h3><p id="password-help" data-v-664bdcda>${ssrInterpolate(unref(copy).passwordHint)}</p></div><div class="account-password__fields" data-v-664bdcda><label class="is-current" data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(unref(copy).currentPassword)}</span><input${ssrRenderAttr("value", unref(passwordForm).currentPassword)} type="password" name="current-password" autocomplete="current-password" maxlength="200" required data-v-664bdcda></label><label data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(unref(copy).newPassword)}</span><input${ssrRenderAttr("value", unref(passwordForm).newPassword)} type="password" name="new-password" autocomplete="new-password" minlength="10" maxlength="200" aria-describedby="password-help" required data-v-664bdcda></label><label data-v-664bdcda><span data-v-664bdcda>${ssrInterpolate(unref(copy).confirmPassword)}</span><input${ssrRenderAttr("value", unref(passwordForm).confirmation)} type="password" name="new-password-confirmation" autocomplete="new-password" minlength="10" maxlength="200" required data-v-664bdcda></label></div>`);
          if (unref(passwordError)) {
            _push(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(passwordError))}</p>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(passwordChanged)) {
            _push(`<p class="account-success" role="status" data-v-664bdcda>${ssrInterpolate(unref(copy).passwordChanged)}</p>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<button type="submit" class="account-button account-button--primary"${ssrIncludeBooleanAttr(unref(passwordChanging)) ? " disabled" : ""} data-v-664bdcda>${ssrInterpolate(unref(passwordChanging) ? unref(copy).changing : unref(copy).changePasswordButton)}</button></form>`);
        } else {
          _push(`<!---->`);
        }
        if (!__props.readOnly) {
          _push(`<div class="account-actions" data-v-664bdcda><article data-v-664bdcda><div data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(unref(copy).deleteResults)}</h3><p data-v-664bdcda>${ssrInterpolate(unref(copy).deleteResultsHint)}</p></div><button type="button" class="account-button account-button--warning" data-v-664bdcda>${ssrInterpolate(unref(copy).deleteResults)}</button></article><article data-v-664bdcda><div data-v-664bdcda><h3 data-v-664bdcda>${ssrInterpolate(unref(copy).deleteAccount)}</h3><p data-v-664bdcda>${ssrInterpolate(text("deleteAccountHint", { username: unref(displayUsername) }))}</p></div><button type="button" class="account-button account-button--danger" data-v-664bdcda>${ssrInterpolate(unref(copy).deleteAccount)}</button></article></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</section>`);
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(accountDialog)) {
          _push2(`<div class="account-dialog-backdrop" role="presentation" data-v-664bdcda><section class="account-dialog" role="alertdialog" aria-modal="true" aria-labelledby="account-dialog-title" aria-describedby="account-dialog-description" data-v-664bdcda><button class="account-dialog__close" type="button"${ssrRenderAttr("aria-label", unref(copy).close)} autofocus${ssrIncludeBooleanAttr(unref(accountActionPending)) ? " disabled" : ""} data-v-664bdcda> × </button><span class="account-dialog__icon" aria-hidden="true" data-v-664bdcda>!</span><h2 id="account-dialog-title" data-v-664bdcda>${ssrInterpolate(unref(accountDialog) === "results" ? unref(copy).deleteAllResultsQuestion : unref(copy).deleteAccountQuestion)}</h2><p id="account-dialog-description" data-v-664bdcda>${ssrInterpolate(unref(accountDialog) === "results" ? text("deleteResultsDialog", { username: unref(displayUsername) }) : text("deleteAccountDialog", { username: unref(displayUsername) }))}</p>`);
          if (unref(accountActionError)) {
            _push2(`<p class="preferences-error" role="alert" data-v-664bdcda>${ssrInterpolate(unref(accountActionError))}</p>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`<div class="account-dialog__actions" data-v-664bdcda><button type="button"${ssrIncludeBooleanAttr(unref(accountActionPending)) ? " disabled" : ""} data-v-664bdcda>${ssrInterpolate(unref(copy).cancel)}</button><button type="button" class="is-danger"${ssrIncludeBooleanAttr(unref(accountActionPending)) ? " disabled" : ""} data-v-664bdcda>${ssrInterpolate(unref(accountActionPending) ? unref(copy).deleting : unref(accountDialog) === "results" ? unref(copy).confirmDeleteResults : unref(copy).confirmDeleteAccount)}</button></div></section></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
...
const __nuxt_component_0 = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main, [["__scopeId", "data-v-664bdcda"]]), { __name: "LearnerSpace" });

export { __nuxt_component_0 as _ };
//# sourceMappingURL=LearnerSpace-Cv_x-UKc.mjs.map
