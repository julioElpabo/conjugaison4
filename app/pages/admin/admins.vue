...
async function loadUsers(keepSelection = true) {
  ...
  error.value = ''
  try {
    const response = await $fetch<{ users: ManagedUser[] }>('/api/admin/admins', { credentials: 'same-origin' })
...
async function saveUser() {
  ...
  try {
    if (wasEditing) {
      ...
      const response = await $fetch<{ id: number }>('/api/admin/admins', { method: 'POST', credentials: 'same-origin', body })
    ...
    <AdminShell>
      <div class="admin-users">
        <header class="admin-section-heading">
          <div><p class="admin-eyebrow">Accès</p><h1>Admins</h1><p class="admin-muted">Créez et gérez les comptes qui peuvent accéder à l’administration.</p></div>
          <button class="admin-button admin-button--primary" type="button" @click="resetDraft">Nouvel admin</button>
        </header>
        ...
        <div class="admin-users__workspace">
          ...
          <form class="admin-users__form admin-card" @submit.prevent="saveUser">
            <div class="admin-users__form-heading">
              <div><p class="admin-eyebrow">{{ editing ? `Compte no ${selectedId}` : 'Nouveau compte' }}</p><h2>{{ editing ? 'Modifier l’admin' : 'Créer un admin' }}</h2></div>
